...
option_list = list(
    ...
    make_option( c("--vismethod","-m"), type= "character",default="vlnplot,featureplot",
                 ...
                 he methods can be ridgeplot,vlnplot,flip_vlnplot,splitby_featureplot,dotplot,featureplot,heatmap,featurebygroup,diff_heatmap,geneset,corscatter"),
    ...
    if ( vismethod == "flip_vlnplot" ){
        ...
        for ( clusterx in unique(topn_markers$folder_suffix) ){
            ...
            p <- VlnPlot(obj, features = feature, pt.size = pt.size, ... ) +
               xlab(groupby) + ylab("") + ggtitle(feature) +
               coord_flip() +
    ...
    if ( vismethod == "splitby_featureplot" ){
        ...
        for (i in topn_markers2vis){ 
            ...
            ggfeature <- lapply(ggfeature, function(x) x
                        + scale_colour_gradientn(colors= c("grey","red")
                                ) )

...
option_list = list(
    ...
    make_option( c("--vismethod","-m"), type= "character",default="vlnplot,featureplot",
                 ...
                 he methods can be ridgeplot,vlnplot,dotplot,featureplot,heatmap,featurebygroup,diff_heatmap,geneset,corscatter."),
    ...
    if ( vismethod == "featureplot" ){
        ...
        #     dev.off()
            ...
            while (i <=  length(ggfeature)){
              ...
              nrow = ceiling(length(ggfeature_sub))
              pdf(file.path(output_dir,paste0("top", "marker_gene_featureplot",ifelse(i==1,"",i%/%50+1),".pdf", collapse = "_")),
                ...
                height = nrow*6)
              grid.arrange(grobs = ggfeature_sub, ncol= ifelse(length(ggfeature_sub)==1,yes = 1, no =1))
              dev.off()
...
#        box=list();feat=list();vln=list()
		vln=list()
        for (geneset in names(topn_markers2vis)){
          ## boxplot
          # box[[geneset]] = ggplot(metadata,aes_string(x=groupby,y=geneset,fill=groupby))+
            # #stat_boxplot(geom = "errorbar",width = 0.5,lwd=0.5) + 
            # geom_boxplot()+
            # labs(x=groupby,y=paste0(geneset,"_score")) +
          ...
          # ggsave(file.path(output_dir,paste0(geneset,"_score_boxplot.pdf")),plot=box[[geneset]],width=8)
          # ggsave(file.path(output_dir,paste0(geneset,"_score_boxplot.png")),plot=box[[geneset]],width=8)
          
          ## vlnplot
          # vln[[geneset]] = ggplot(metadata,aes_string(x=groupby,y=geneset,fill=groupby))+
            # geom_violin()+
            # labs(x=paste0(groupby),y=paste0(geneset,"_score")) +
            # theme(panel.grid.major =element_blank(),
                  # panel.grid.minor = element_blank(),
                  # panel.background = element_blank(),
                  # axis.line = element_line(color = "black"))+
          ...
          colors2use = CustomCol2(1:length(unique(seurat_ob@meta.data[,groupby])))
          vln[[geneset]] = VlnPlot(seurat_ob, features = geneset, cols = colors2use,x.lab.rot = T,
                                                            pt.size = pointsize, alpha = opt$alpha2use,
                                                            group.by = groupby, split.by = splitby)+
															labs(y = paste0(geneset,"_score")) + 
															stat_summary(fun.y="median",geom='point',size=3 ,aes(x=ident,color=split))+ 
...
if ( vismethod == "corscatter" ){
    ...
    if ( vismethod == "dotplot" ){
        for ( clusterx in unique(topn_markers$folder_suffix) ){
            ...
            ggsave(file.path(output_dir,paste0("top", "marker_gene_dotplot.png", collapse = "_")),dpi = 1000 ,limitsize = F ,width=(0.3*length(topn_markers2vis)+2.5+max(nchar(names(table(seurat_ob@meta.data[,groupby]))))/10))
        }
    }

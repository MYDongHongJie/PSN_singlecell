...
LRNetwork<-function(
  ...
  set.seed(1234)
  datax=data
  netx<-datax %>% group_by(ligand_cell, receptor_cell, group) %>% dplyr::summarize(n=n())
  netx$width <- 1+edge.max.width/(max(netx$n)-min(netx$n))*(netx$n-min(netx$n))
  data <- subset(datax, group == groupx)
  net = subset(netx, group == groupx)
...
#  E(g)$color<-V(g)$color[edge.start[,1]]
  ...
  plot(g,edge.curved=edge.curved,vertex.shape=shape,layout=layout.circle,margin=margin,edge.arrow.size=edge.arrow.size,main=unique(E(g)$group))
  return(g)
}
...
#  expr_range=c(-15,-6,3),
  ...
  data <- data %>% group_by(receptor_cell,ligand_cell, group) %>% filter( pval < opt$pvalue ) %>% top_n(topn, !!ensym(screenvar))
  data <- as.data.frame(data)
  max_range = max(ceiling(max(data$ligand_expr)),ceiling(max(data$receptor_expr)))
  min_range = min(floor(min(data$ligand_expr)), floor(min(data$receptor_expr)))

  expr_range = c(min_range, (max_range+min_range)/2, max_range)
  data = subset(data, group == groupx)
...
#                    type = "lines", legend_gp = gpar(lty = 1:2))
  ...
  pushViewport(viewport(x = lgd_pos, y = 0.5))
...
LRBarplot <- function(
  ...
  ## significant summary
  plot_data = data %>% filter(pval < 0.05) %>% group_by(ligand_cell, receptor_cell, group ) %>%
...
#      scale_fill_discrete(name="Cell types of receptors") +
      scale_fill_manual("Cell types of receptors", values = colx) + facet_wrap(~group) +
...
if ( is.null(data$group) ) {
    data$group = "cell_comm"
    print("No grouping information in the input file, and the overall data is analyzed by default!")
    # stop("No grouping information in the input file, check the file or select another script!")
}
...
for ( plotx in unlist( strsplit(opt$plot, ",", perl = T) ) ){
  if ( plotx == "dotplot" ){
    ...
    data$ligand_cell = paste0(datax$ligand_cell,"(",datax$group,")") 
    }
  ...
  if ( plotx == "network" ){
    ...
    colx = SelectColors(palette = opt$colorschema, length(celltypes))
    pdf(file.path(output_dir, "cell_comm_network.pdf"),width = length(unique(data$group))*10, height = 10)
    par(mfrow = c(1,length(unique(data$group))))
    for (groupx in unique(data$group)){
    LRNetwork( data %>% filter(pval < opt$pvalue), groupx = groupx,,
              col = colx, edge.label.cex = 0.3, 
              edge.max.width = 5, arrow.width = 0.5, 
              vertex.label.cex = 0.5, vertex.size = 10)
	}
    dev.off()
    png(file.path(output_dir, "cell_comm_network.png"), width = length(unique(data$group))*10, height = 10, res = 500, units = "in" )
    par(mfrow=c(1,length(unique(data$group))))
    for (groupx in unique(data$group)){
    LRNetwork( data %>% filter(pval < opt$pvalue), groupx =  groupx,
              col = colx, edge.label.cex = 0.3, 
  ...
  if ( plotx == "circos" ){
    ...
    celltypes = unique(c(data$receptor_cell, data$ligand_cell))
    opt$topn = ifelse( length(celltypes)<7, opt$topn, 3)
    colx = SelectColors(palette = opt$colorschema, length(celltypes))
    pdf( file.path(output_dir, "cell_comm_circos_plot.pdf"), width = length(unique(datax$group))*10+10, height = 11)
    par(mfrow = c(1,length(unique(datax$group))))
    for (groupx in c(1:length(unique(datax$group)))){
    # data = subset(datax, group==unique(datax$group)[groupx])
    lgd_pos = ifelse(length(unique(datax$group))==1, 0.75, groupx/length(unique(datax$group))+0.01)
    LRCircos( data, unique(datax$group)[groupx], lgd_pos, gap.degree = 0.05, cell_col = colx, screenvar = opt$topby,
             topn = opt$topn, labels.cex = 0.35,link.lwd=1, arr.length = 0.1)
    title(main = unique(datax$group)[groupx])
    }
    dev.off()
    png( file.path(output_dir, "cell_comm_circos_plot.png"), width = length(unique(datax$group))*10+10, height = 11, res = 500, units = "in" )
    par(mfrow = c(1,length(unique(datax$group))))
    for (groupx in c(1:length(unique(datax$group)))){
    # data = subset(datax, group==unique(datax$group)[groupx])
    lgd_pos = ifelse(length(unique(datax$group))==1, 0.75, groupx/length(unique(datax$group))+0.01)
  ...
  if ( plotx == "chorddiagram" ){
    data = datax
    if (length(unique(data$group)) > 1){
    data$ligand_cell = paste0("(",data$group,")",data$ligand_cell)
    data$receptor_cell = paste0("(",data$group,")",data$receptor_cell)
    }
    suppressPackageStartupMessages( library("circlize") ) 
    pdf( file.path(output_dir, paste0("cell_comm_chorddiagram_plot.pdf") ))
	par(cex = 0.6, mar = c(0, 0, 0, 0))
    LRChorddiagram(data, grid_orbit_h=0.02,label_h=0.04,diffHeight=4,output_dir=output_dir)
    dev.off()
    png( file.path(output_dir, paste0("cell_comm_chorddiagram_plot.png")), width = 7, height = 7, res = 300, units = "in" )
	par(cex = 0.6, mar = c(0, 0, 0, 0))
    LRChorddiagram(data, grid_orbit_h=0.02,label_h=0.04,diffHeight=4,output_dir=output_dir)
    dev.off()
    circos.clear()
  ...
  if( plotx == "barplot"){
    ...
    ggsave(file.path(output_dir, "cell_comm_histogram_plot.pdf"), plot = out, height= ceiling(length(unique(data$group))/2)*7, width = ifelse( length(celltypes)<7,length(unique(data$group))*5+2,length(unique(data$group))*length(celltypes)))
    ggsave(file.path(output_dir, "cell_comm_histogram_plot.png"), plot = out, height= ceiling(length(unique(data$group))/2)*7, width = ifelse( length(celltypes)<7,length(unique(data$group))*5+2,length(unique(data$group))*length(celltypes)), dpi=1000)
  }

}
if(!file.exists(file.path(output_dir, "cellphoneDB细胞通讯分析说明.docx"))){
  file.copy("/public/scRNA/works/Documents/cellphoneDB细胞通讯分析说明.docx",

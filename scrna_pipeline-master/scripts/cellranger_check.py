...
def cellarnger_check(stat,html,cellranger_only,Estimated_Number_of_Cells,Q30_Base_in_RNA_Read,Fraction_Reads_in_Cells,Median_Genes_per_Cell,Number_of_Reads,Reads_Mapped_Confidently_to_Transcriptome,Reads_Mapped_to_Genome,Project_Num,Species,Customer,Project_management,Laboratory,raw_data_obs_address,include_introns):
    """
    ...
    if cell_big_error_sampleid:
       ...
       cell_big_error =   '\n    如下图所示：' + \
           big_samples + " 样本" + "细胞数高于 " + str(Est_Num_Cells_max_stand) + ", 需进行force cell（若自己无法判断请支持协助判断） \n"
       info += cell_big_error

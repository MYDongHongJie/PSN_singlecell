...
import sys,shutil,os,click,subprocess,sys,re,time,yaml,re,shlex
...
def scrna_report(program_path, configfile, rds):
	...
	module_num = 0
	## ========================= 1.1 cellranger =========================
	if not os.path.exists("cellranger" ):
		logging("[cellranger module] × ",log_file,WARNING)
		import csv
		...
		module_num += 1
		logging("[cellranger module] √ ",log_file)
		Sample_Num = config['report']['Sample_Num']
		sample_name = [str(i).split('/')[-3] for i in glob("cellranger/*/outs/filtered_feature_bc_matrix" )]
		file_number ={i: len(os.listdir("cellranger/"+i+"/outs/")) for i in sample_name }
		if any([v!= 9 and v !=11 for k,v in file_number.items()]):
			logging("[cellranger module] cellranger/ 目录下各样本文件数 :" +  str(file_number) + "；每个样本应有9或11个文件。默认为外来数据不提供cellranger结果。Skipping... \n[cellranger module] × " ,log_file,WARNING)
			module_num -= 1
		else:
			if len(sample_name) != Sample_Num:
				logging_exit("[cellranger module] Number of samples(from config.yaml) does not match the number of cellranger directory!",log_file)
			else:
				# each sample
				for j in sample_name:
					os.makedirs(f"{outdir}/{module_num}.CellRanger/{j}" )
					subprocess.call(f'ln -s {program_path}/cellranger/{j}/outs/* {outdir}/{module_num}.CellRanger/{j} ', shell=True)
					if os.path.exists(f"{outdir}/{module_num}.CellRanger/{j}/analysis/gem_classification.csv" ):
						csv_file = shlex.quote(f"{outdir}/{module_num}.CellRanger/{j}/analysis/gem_classification.csv")
						dst_folder = shlex.quote(f"{outdir}/{module_num}.CellRanger/{j}/")
						subprocess.call(f'cp {csv_file} {dst_folder}', shell=True)
						subprocess.call(f'rm -r {outdir}/{module_num}.CellRanger/{j}/analysis', shell=True)
					...
					# png
					check_exist(
						file=f"cellranger/{j}.png",
						destination= f"{outdir}/{module_num}.CellRanger/" ,
						log_file=log_file
					)
				# aggr
				if len(sample_name) > 1 :
					if not os.path.exists("cellranger/aggr" ):
						...
						subprocess.call(f'cp -r  {program_path}/cellranger/aggr/outs/* {outdir}/{module_num}.CellRanger/aggr ', shell=True)
						subprocess.call(f'rm  {outdir}/{module_num}.CellRanger/aggr/aggregation.csv ', shell=True)
						subprocess.call(f'rm  -r {outdir}/{module_num}.CellRanger/aggr/count/analysis', shell=True)
	## ========================= 1.2 Count_QC =========================
	if not os.path.exists("Count_QC"):
		...
		## test 2
		# file_num_tmp = [check_exist(f"{program_path}/Count_QC/{f}",f"{outdir}/{module_num}.Count_QC/",log_file) 
		# 		for f in os.listdir(f"{program_path}/Count_QC/") 
		# 		if re.search(r'.*\.(pdf|png|csv|xls)$', f)]
		# if not len(file_num_tmp) == 6 :
		# 	logging_exit("[Count_QC module] Count_QC 目录下 %s 个文件, 应有 6 个文件!" %(len(os.listdir("%s/%s.Count_QC/" % (outdir, module_num)))) ,log_file)
		## test 1
		# subprocess.call('cp  %s/Count_QC/* %s/%s.Count_QC/' % (program_path, outdir, module_num), shell=True)
		# if os.path.exists("%s/%s.Count_QC/filtered.h5seurat" % (outdir, module_num)):
		# 	subprocess.call('rm %s/%s.Count_QC/filtered.h5seurat' % (outdir, module_num), shell=True)
		#  if not len(os.listdir("%s/%s.Count_QC/" % (outdir, module_num))) == 6 :
		#  	logging_exit("[Count_QC module] Count_QC 目录下 %s 个文件, 应有 6 个文件!" %(len(os.listdir("%s/%s.Count_QC/" % (outdir, module_num)))) ,log_file)
	## ========================= 1.3 Clustering ========================= 
	if not os.path.exists("Clustering" ):
		...
		if len(sample_name) >1 :
			check_exist(file="Clustering/visualize_cluster_by_clusters",
				destination=f'{outdir}/{module_num}.Clustering/',
				log_file= log_file)
		# logging("[visualize_cluster_by_clusters module] √ ",log_file) 
		# os.makedirs(f'{outdir}/{module_num}.visualize_cluster_by_clusters/')
			# check_exist(file="Clustering/visualize_cluster_by_clusters",
				# destination=f'{outdir}/{module_num}.Clustering/',
				# log_file= log_file)
		## correlation
	# if not os.path.exists("clusters_correlation" ):
		# logging("[clusters_correlation module] × ",log_file,WARNING) 
	# else:
		module_num += 1
	...
	if not os.path.exists("Diffexp" ):
		...
		if not os.path.exists("Diffexp/enrichment"):
			...
			subprocess.call(f'cp -r Diffexp/enrichment/background_files/ {outdir}/{module_num}.enrichment/', shell=True)
	...
	subprocess.call(f'rm -f {outdir}/{module_num}.supplemental_material/10x_单细胞转录组报告树状图.pdf', shell=True)
	subprocess.call(f'cp -r {scriptdir}/supplemental_material/10x_单细胞转录组报告树状图.pdf {outdir}/', shell=True)
...
####################################################################################################
	...
	header_info = {key: config["report"][key] for key in config["report"] if
               key in ["Project_Num", "Customer", "Species", "Sample_Num", "Library", "Task_Num"]}
	...
	header_info["文库类型"] = header_info.pop("Library")
	header_info["任务单号"] = header_info.pop("Task_Num")
	report = Report('10x单细胞转录组项目结题报告', title = '10x单细胞转录组项目结题报告', header_info = dict(header_info), oe_welcome='''
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
	...
	## ========================= 2.4.1 基因定量质控 =========================  
	if len(list(glob("*.CellRanger"))) != 0:
		report_hyperlink_list.append('[样本 Cell Ranger 质控结果](#cellranger)\n\n')
		Cellranger = result_module.add_section('基因定量质控')
		Cellranger_plots = sorted(list(glob('*.CellRanger/*.png')),reverse=False)
		Cellranger_plots = list(zip(Cellranger_plots, ['样本细胞质量统计结果']*len(Cellranger_plots)))
		Cellranger.add_plot(Cellranger_plots, caption = '样本细胞质量统计结果')
		Cellranger.add_table('scRNA_qc_stat.txt', caption = '样本细胞质量统计结果说明')
	...
	if len(list(glob("*.Count_QC"))) != 0:
		...
		link = QC_folder
		Count_QC.add_comment('''结果目录：[{link}]({link})''',link=link)
...
本次分析共完成 {len(summ)} 个样本单细胞转录组测序,
各样本 Cell Ranger 定量质控的高质量细胞数分布在 {min(summ['Total_cells_beforeQC'])} ~ {max(summ['Total_cells_beforeQC'])} 个，
...
每个细胞中的平均基因数分布在 {format(min(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')} ~ {format(max(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')}''')
		...
		if mito:
			if config['report_params']['percent_mito'] is not None and int(float(config['report_params']['percent_mito'])) == 1:
				mito_description=f'、线粒体UMI数高于所有细胞中线粒体UMI中位数的四倍'
			...
			elif config['report_params']['percent_mito'] is None:
				...
				mito_description=f'、线粒体UMI占比高于 {percent_mito}%'
				mito_show=f'、线粒体UMI所占比例（percent_mito）'
				# summary
				min_afterQC_mito = format(min(summ['mean_percent.mito_after'+lastQCstep].astype(float)),'.4f') # mito
				max_afterQC_mito = format(max(summ['mean_percent.mito_after'+lastQCstep].astype(float)),'.4f') 
		...
		if HB:
			...
			HB_description=f'、血红细胞基因占比高于 {percent_HB}%的细胞'
		...
		try:
			...
			QC_description = f'''在 Cell Ranger 初步质控的基础上进一步对实验数据进行质控，剔除多细胞、双细胞或者未结合上细胞的数据，进行下游分析。
\n本项目中的质控标准为：剔除基因数小于 {format(min(summ['nFeature_RNA_lower'].astype(float)),'.0f')} 、UMI数小于 {format(min(summ['nCount_RNA_lower'].astype(float)),'.0f')}、log10GenesPerUMI小于 {min(summ['log10GenesPerUMI_lower'].astype(float))}{mito_description}{HB_description}作为高质量细胞{rmdoublet_description} 
\n质控前后每个细胞的基因数量（nGene）、UMI数量（nUMI）、单位UMI内基因数目的比例（log10GenesPerUMI）{mito_show}{HB_show}的小提琴图展示如下：
		'''
		else:
			QC_description = f'''在 Cell Ranger 初步质控的基础上进一步对实验数据进行质控，剔除多细胞、双细胞或者未结合上细胞的数据。{roboust_linear_model_description}
...
录本数目异常过多的细胞很可能是由于对应的油包水微滴中包含多个细胞，需要通过设置合理的阈值将其过滤''')
	...
	if len(list(glob("*.Clustering"))) != 0:
		...
		if reduct1 == "pca":
			...
			REDUCT1 = "Harmony"
			reduct_module.add_comment(f'''由于本项目不同样本间存在批次，首先采用了{REDUCT1} 的降维算法剔除批次效应，再基于降维结果通过 {REDUCT2} 算法对单细胞群聚类进行可视化，聚类算法采用SNN，最终获得最优细胞分群。''')
		else:
			REDUCT1 = "MNN（mutual nearest neighbors, 互享最近邻）" 
		...
		if os.path.exists(f"{Clustering_folder}/visualize_cluster_by_clusters" ):
			...
			# groupby_module.add_comment(f'''样本间/分组间降为聚类结果，是通过聚类或细胞占比的统计结果来体现各cluster在不同样本和分组中的差异。对于同一个组的重复样本来说，可以评估重复样本的好坏或个体差异情况。对于不同分组的样本，可以评估组间差异的细胞是哪些，以便锁定后进一步挖掘。''')
			...
			# groupby_module.add_comment(f'''每个Cluster中各样本来源细胞占比图，可以反映各样本中细胞分布的差异。''')
			...
			groupby_contents = ['样本间细胞群占比柱状图。图片说明：横坐标表示不同样本，纵坐标表示不同组别中细胞数目所占的百分比。每个样本/分组中，各cluster来源细胞的比例图，可以反映两个信息：1）同一个组的不同重复样本是否一致；2）各cluster在组间的差异。此外，如果样本间细胞数差异比较大的情况，建议用如上柱状图，通过比例（而非细胞数）来分析差异性。']
			...
			# groupby_module.add_comment(f'''每个样本/分组中，各cluster来源细胞的比例图，可以反映两个信息：1）同一个组的不同重复样本是否一致；2）各cluster在组间的差异。此外，如果样本间细胞数差异比较大的情况，建议用如上柱状图，通过比例（而非细胞数）来分析差异性。''')
		...
		link = Clusters_correlation_folder
		Clusters_correlation_module.add_comment('''结果目录：[{link}]({link})''',link=link)
	...
	if len(list(glob("*.Marker"))) != 0:
		...
		link = Marker_folder
		Marker_module.add_comment('''结果目录：[{link}]({link})''',link=link)
	...
	if len(list(glob("*.Reference_celltype"))) != 0:
		...
		link = celltype_folder[0]
		Celltype_module.add_comment('''结果目录：[{link}]({link})''',link=link)
		# check cluster number
	...
	if len(list(glob("*.Diffexp"))) != 0:
		...
		link = Diffexp_folder[0]
		Diff_module.add_comment('''结果目录：[{link}]({link})''',link=link)
		...
		link = enrichment_folder[0]
		background_link = f'{link}/background_files'
		enrich_module.add_comment('''本次富集分析使用的背景文件见[{background_link}]({background_link}) ，用此背景文件可以导入[云平台富集分析小工具](https://cloud.oebiotech.cn/task/detail/enrichment-oehw/)，进行灵活调整分析。''',background_link=background_link)
		enrich_module.add_comment('''结果目录：[{link}]({link})''',link=link)
		#定义函数获取比较组名称
...
# [^topGO]:Alexa A, Rahnenfuhrer J. topGO: enrichment analysis for gene ontology. R package version 2.8,2010.''')
	...
	## ========================= 2.1.2 项目快捷链接 =========================
	link_faq = '10.supplemental_material/单细胞转录组常见问题(FAQ).pdf'
	link_down = '10.supplemental_material/单细胞转录组项目数据挖掘建议.pdf'
	link0 = '10.supplemental_material/Loupe-Browser使用教程.html'
	link_video = 'https://www.bilibili.com/video/BV1cz4y1y7rs'
	link_Directory = '10x_单细胞转录组报告树状图.pdf'
	...
	report_Directory_list.append(f'''[10x_单细胞转录组报告树状图]({link_Directory})\n''' )
	report_Directory = ''.join(report_Directory_list) 
	project_module.add_section(name="项目快捷链接",description=report_hyperlink)#anchor: linkage
	project_module.add_section(name="项目目录结构",description=report_Directory)#anchor: linkage
	...
	## ========================= 2.5.2 实验技术方法说明 ========================= 
	link1 = '10.supplemental_material/欧易生物单细胞转录组实验技术方法说明_中文.pdf'
	link2 = '10.supplemental_material/欧易生物单细胞转录组实验技术方法说明_英文.pdf'
	appendix_module.add_section('实验技术方法说明',link1=link1,link2=link2)

	## ========================= 2.5.3 生信分析方法说明  ========================= 
	link3 = '10.supplemental_material/欧易生物单细胞转录组生信分析方法_中文.pdf'
	link4 = '10.supplemental_material/欧易生物单细胞转录组生信分析方法_英文.pdf'
	...
	## ========================= 2.5.6 AI修图使用说明 ========================= 
	link_ai = '10.supplemental_material/AI使用说明.html'
	...
	database = pd.read_table('../../../scripts/report/pic/tables/scRNA_database.txt', sep='\t')
	...
	software["Version"][0] = cellranger_version
	appendix_module.add_section('数据分析软件')
	appendix_module.add_table(software, caption = '使用软件及版本')

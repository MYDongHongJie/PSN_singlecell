...
####################################################################################################
    ...
    report = Report('10x单细胞转录组项目Cellranger结题报告', title = '10x单细胞转录组项目Cellranger结题报告', header_info = dict(header_info), oe_welcome='''
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
    ...
    ## ========================= 2.1 SECTION1 项目概况 ========================= 
    ############################################################################
    summ = pd.read_csv("../../cellranger_sum/metrics_summary_stat.csv",index_col=0, sep=',', thousands=',')
    if summ.shape[0] > 1 :
        QC_description = f'''本次分析完成 {summ.shape[0]} 个样本的单细胞转录组测序，各样本 Cellranger 定量质控的高质量细胞数分布在 {min(summ['Estimated Number of Cells'])} ~ {max(summ['Estimated Number of Cells'])} 个'''
    else:
        QC_description = f'''本次分析完成 {summ.shape[0]} 个样本的单细胞转录组测序，各样本 Cellranger 定量质控的高质量细胞数分布在 {min(summ['Estimated Number of Cells'])} 个'''
    ...
    ## ========================= 2.3 SECTION3 生信分析流程 ========================= 
    ###############################################################################
    analysis_module = report.add_section('Cellranger分析流程')
    analysis_module.add_comment('Cellranger分析流程图如下：')
    analysis_module.add_plot('Cellranger.png', caption = 'Cellranger分析流程',content='Cellranger分析流程图')
    ###############################################################################
    ## ========================= 2.4 SECTION4 项目分析结果 ========================= 
    ###############################################################################
    ...
    if len(list(glob("*.CellRanger"))) != 0:
        ...
        Cellranger.add_table('../../../scripts/report/pic/tables/scRNA_qc_stat.txt', caption = '样本细胞质量统计结果说明')

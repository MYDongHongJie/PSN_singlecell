...
bam1=$dir_path/outs/${base_name}.bam
bam2=$dir_path/outs/${base_name}.bam.bai
bam3=$dir_path/outs/${base_name}.bam.csi
...
bam_files2=$(find "${dir_path}/outs/${sample_id}/" -type f -name "*.bam")

if { test -f "$bam1" && test -f "$bam2"; } || { test -f "${bam_files2}" && test -f "${bam_files2}.bai" && test -f "${dir_path}/outs/${project_id}_md5.xls"; }; then
# if test -f "$bam1" &&  test -f "$bam2" ; then
  ...
  if  test -f "${bam_files2}" && test -f "${bam_files2}.bai" && test -f "${dir_path}/outs/${project_id}_md5.xls"; then
    echo "已生成md5，直接上传"
  else
    mkdir $sample_id
    mv $bam1 ${sample_id}/${base_name}.bam
    mv $bam2 ${sample_id}/${base_name}.bam.bai
  ...
  scp -r ${dir_path}/outs/${sample_id}/ ${remote_host}:/cds/scRNA-bambak/${project_id}/
  ...
  if [ $? -eq 0 ]; then
      ...
      if [ -z "${zero_kb_files}" ]; then
          ...
          rm -r "${dir_path}/outs/${sample_id}/"
          # echo "rm -r ${dir_path}/outs/${sample_id}/"

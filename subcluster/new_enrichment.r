...
PlotGo <- function(out_df,outDir){
    ...
    p<- ggplot(data = out_df) +
      ...
      ylab("-log10(P-value)")+coord_flip()
    ggsave(paste(outDir,"GO_enrichment_pvalue_barplot.pdf",sep="/"), width = 8, height = 4)
    ggsave(paste(outDir,"GO_enrichment_pvalue_barplot.png",sep="/"), width = 8, height = 4)
...
PlotKegg <- function(out_df,outDir){
    ...
    q<-qplot(rich,Pathway,data=out_df,colour= Pvalue,size=Number,main="KEGG Enrichment")+
      scale_colour_gradient(low="red",high="green")+
      theme(panel.background=element_rect(fill="white",color="black"),panel.grid.major=element_line(color="grey80",linetype="dotted"))
    ggsave(paste(outDir,"KEGG.richfactor.pdf",sep="/"),width = 6, height = 4)
    ggsave(paste(outDir,"KEGG.richfactor.png",sep="/"),width = 6, height = 4)

    out_df=out_df[order(out_df$Category),]
    out_df$Category <- factor(out_df$Category, levels = unique(out_df$Category))
    out_df$Pathway <- factor(out_df$Pathway, levels = out_df$Pathway)
    p<- ggplot(data = out_df) +
      geom_col(aes(x = Pathway, y = -log10(as.numeric(Pvalue)), fill = Pvalue),width =0.8) +
      scale_color_brewer(type="seq",palette="Dark2")+
      theme( strip.text.y = element_text(angle = 0),axis.text.x=element_text(size=10,angle=80,hjust=1)) +
      ylab("-log10(P-value)") +coord_flip()
    ggsave(paste(outDir,"KEGG_enrichment_pvalue_barplot.pdf",sep="/"), width = 8, height = 4)
    ggsave(paste(outDir,"KEGG_enrichment_pvalue_barplot.png",sep="/"), width = 8, height = 4)
}

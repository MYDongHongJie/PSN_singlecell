...
format_BDVDJ <- function(AIRR_df,out_type){
  ...
  VDJ_Dominant_AIRR <- read.delim((AIRR_df),sep="\t",comment.char = '#')
...
#format_vdj_cells <- function(AIRR_df,out_type,sampleid){
#     VDJ_Dominant_AIRR <- read.delim((AIRR_df),sep="\t",comment.char = '#')
#    filter_colname = c("cell_id","locus","consensus_count","duplicate_count","productive","cdr3","cdr3_aa","v_call","v_sequence_start","v_sequence_end","d_call","d_sequence_start","d_sequence_end","j_call","j_sequence_start","j_sequence_end",
#                       names(VDJ_Dominant_AIRR)[grep(out_type, names(VDJ_Dominant_AIRR), ignore.case = FALSE)])
#    filter_Cell = VDJ_Dominant_AIRR[filter_colname]
#    filter_Cell$sampleid = sampleid
#    filter_Cell_True = filter_Cell[filter_Cell$productive=="True",]
#    filter_Cell_True[filter_Cell_True == ""]<-NA
#    return(filter_Cell_True)
#}
...
for (sample in metadata$sampleid){
	...
	VDJ_Dominant_AIRR = file.path(opt$inputdir,sample,paste0(sample,"_VDJ_Dominant_Contigs_AIRR.tsv"))
...
##3.get counts from VDJ_perCell.csv
...
#		print(i)
#		out_type = i
#		out_file = format_vdj_cells(VDJ_Dominant_AIRR,out_type,sampleid)
#		write.table(out_file,file.path(outdir,paste0(sampleid,"_",i,"_contig_annotation.xls")),sep="\t",quote=F,row.names=F,col.names=T)
#		}
#}

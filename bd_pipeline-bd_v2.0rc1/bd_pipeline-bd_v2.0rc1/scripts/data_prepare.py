    ...
    for sample in sample_list:
        if sample in os.listdir(f"{outdir}/"): 
            ...
            for i in {"1", "2"}:
                ...
                fq_list = sorted([item for sublist in [glob( f'{outdir}/{sample}'+ ext) for ext in [f"/*R{i}.fastq.gz", f"/*_{i}.fq.gz", f"/*R{i}_001.fastq.gz",f"/*.R{i}.fq.gz"]] for item in sublist])
                j = 1
                for fastq in fq_list:
                    #src =  fastq.replace(f"{outdir}/{sample}","..")
                    src =  fastq
                    dst = f'{outdir}/{sample}/{sample}_fastqs/{sample}.R{i}.fastq.gz'
                    fastq_show=fastq.replace(f"{outdir}/","")
                    des_show=f"{sample}/{sample}_fastqs/{sample}.R{i}.fastq.gz"
...
def run(configfile,outdir):
    ...
    samples_config = config["BD_params"]["samples"]
    ## get sample names
    if type(samples_config) == list:
        ...
        sys.exit("\033[31;1m"+"ERROR: BD_params:samples can only be string 'all' or a list of sample names."+"\033[0m")

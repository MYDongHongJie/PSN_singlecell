...
if (substring(packageVersion("Seurat"),1,1) == 4) {
    ...
    if("ADT" %in% names(data_ob@assays)){
        ADT_list=list()
        ADT_list$counts =  data_ob@assays$ADT@counts
        ADT_list$data =  data_ob@assays$ADT@data
        ADT_list$scale.data =  data_ob@assays$ADT@scale.data
    }
    #commands
    ...
    if("ADT" %in% names(data_ob@assays)){
        saveRDSMC(ADT_list,file.path(output_dir,"ADT_list.rds"))
    }
    ...
    if ( file.exists(file.path(output_dir,"ADT_list.rds")) ){
        ADT_list= readRDSMC(file.path(output_dir,"ADT_list.rds"),20)
        data_ob[["ADT"]] <- CreateAssayObject(counts = ADT_list$counts)
        data_ob@assays$ADT@data = ADT_list$data
        data_ob@assays$ADT@scale.data = ADT_list$scale.data
    }
    # reduction
    reductions = list()

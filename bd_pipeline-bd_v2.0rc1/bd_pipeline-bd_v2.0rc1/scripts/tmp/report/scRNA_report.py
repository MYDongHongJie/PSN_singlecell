...
def scrna_report(program_path, configfile, rds):
	...
	if not os.path.exists("Diffexp" ):
		...
		if not os.path.exists("Diffexp/enrichment"):
			...
			os.makedirs(f"{outdir}/{module_num}.Diffexp/enrichment")
			subprocess.call(f'cp -r Diffexp/enrichment/GO_enrichment/ {outdir}/{module_num}.Diffexp/enrichment/', shell=True)
			subprocess.call(f'cp -r Diffexp/enrichment/KEGG_enrichment/ {outdir}/{module_num}.Diffexp/enrichment/', shell=True)
			if not len(os.listdir(f"{outdir}/{module_num}.Diffexp/enrichment/GO_enrichment")) == ndiffexp + 1:
				logging(f"GO_enrichment 目录下 {len(os.listdir(f'{outdir}/{module_num}.Diffexp/enrichment/GO_enrichment'))} 个文件，应有 {ndiffexp + 1} 个文件! 请核查!!!",log_file,WARNING)
			else:
				for i in glob(f"{outdir}/{module_num}.Diffexp/enrichment/GO_enrichment/*-vs-*/"):
					if len(os.listdir(i)) != 30:
						logging(f"GO_enrichment 目录下 {str(i).split('GO_enrichment/')[-1]} 分组有 {len(os.listdir(i))} 个文件，应有 30 个文件! 请核查!!!",log_file,WARNING)
			if not len(os.listdir(f"{outdir}/{module_num}.Diffexp/enrichment/KEGG_enrichment")) == ndiffexp + 1:
				logging(f"KEGG_enrichment 目录下 {len(os.listdir(f'{outdir}/{module_num}.Diffexp/enrichment/KEGG_enrichment'))} 个文件，应有 {ndiffexp + 1} 个文件! 请核查!!!",log_file,WARNING)
			else:
				for i in glob(f"{outdir}/{module_num}.Diffexp/enrichment/KEGG_enrichment/*-vs-*/"):
					if len(os.listdir(i)) != 27:
						logging(f"KEGG_enrichment 目录下 {str(i).split('KEGG_enrichment/')[-1]} 分组有 {len(os.listdir(i))} 个文件，应有 27 个文件! 请核查!!!",log_file,WARNING)
		## ========================= 1.8 PPI =========================  
		if not os.path.exists("Diffexp/ppi" ):
			logging("[ppi module] × ",log_file,WARNING) 
		else: 
			logging("[ppi module] √ ",log_file)
			os.makedirs(f"{outdir}/{module_num}.Diffexp/ppi")
			check_file_num("Diffexp/ppi", f"{outdir}/{module_num}.Diffexp/ppi",ndiffexp*8+1,log_file)
			subprocess.call(f'rm {outdir}/{module_num}.Diffexp/ppi/color_table.tsv', shell=True)
			subprocess.call(f'rm -f {outdir}/{module_num}.Diffexp/ppi/*mapping_results.tsv', shell=True)
...
每个细胞中的平均基因数分布在 {format(min(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')} ~ {format(max(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')}''')
		...
		if mito:
			...
			mito_description=f'、线粒体UMI占比低于 {percent_mito}%'
...
录本数目异常过多的细胞很可能是由于对应的油包水微滴中包含多个细胞，需要通过设置合理的阈值将其过滤''')
	...
	if len(list(glob("*.Clustering"))) != 0:
		...
		if os.path.exists(f"{Clustering_folder}/visualize_cluster_by_clusters" ):
			...
			groupby_module.add_plot(groupby_plots3, caption = '细胞群样本占比柱状图', description ='图片说明：横坐标表示不同细胞群，纵坐标表示不同组别中细胞数目所占的百分比。')
			...
			groupby_module.add_plot(groupby_plots4, caption = '样本间细胞群占比柱状图', description ='图片说明：横坐标表示不同样本，纵坐标表示不同组别中细胞数目所占的百分比。')
		...
		Clusters_correlation_folder = list(glob("*.Clustering/clusters_correlation"))[0]
		Clusters_correlation_plots = list(glob('*.Clustering/clusters_correlation/coefficient_heatmap.png'))
	...
	if len(list(glob("*.Diffexp"))) != 0:
		...
		## ========================= 2.4.8 差异基因GO富集分析 ========================= 
		enrichment_folder = list(glob(r"[0-9]*Diffexp/enrichment"))
...
# [^topGO]:Alexa A, Rahnenfuhrer J. topGO: enrichment analysis for gene ontology. R package version 2.8,2010.''')
		...
		## ========================= 2.4.10 差异基因PPI富集分析 =========================
		ppi_folder = list(glob(r"[0-9]*Diffexp/ppi"))
		if len(ppi_folder)>0: 

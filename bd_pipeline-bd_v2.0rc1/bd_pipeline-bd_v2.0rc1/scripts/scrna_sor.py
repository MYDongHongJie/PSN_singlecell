...
def project(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    desc.pop("Reference")
    desc["Rawdata_Path"] = ','.join(map(str, yaml_file["BD_params"]["raw_data_obs_address"]))
    # if yaml_file["BD_params"]["include_introns"]==True:
...
def QC(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    '''
    ## get sample names
    if type(yaml_file["BD_params"]["samples"]) == list:
        if all(isinstance(i, str) for i in yaml_file["BD_params"]["samples"]):
            sample_list = yaml_file["BD_params"]["samples"]
        elif all(isinstance(i, dict) for i in yaml_file["BD_params"]["samples"]):
            sample_list = [v for i in yaml_file["BD_params"]["samples"] for v,k in i.items()]
        samples = pd.Series(sample_list,index = sample_list)
    elif type(yaml_file["BD_params"]["samples"]) == str and yaml_file["BD_params"]["samples"] == "all" :
            samples = pd.Series()
            for addr in yaml_file["BD_params"]["raw_data_obs_address"]:
    ...
    else :  sys.exit("BD_params - samples can only be string 'all' or a list of sample names.")
    # report - Count_QC
    ...
    for (sample) in samples.index:
        ...
        QC=pd.read_csv(f"result/cellranger/{sample}/outs/metrics_summary.csv", dtype=str)
        # rename and reorder 
        ...
        desc["Fraction_Reads_in_Cells(%)"] = desc.pop("Fraction_Reads_in_Cells")
...
def software(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    :return:
    '''
    if yaml_file["BD_params"]["module"]["BD_analysis"] == True:
        logfile=os.path.join(logdir,"cellranger_software_upload_success.txt")
        if not os.path.exists(logfile):
            desc=dict()
            desc["Version"] = yaml_file['BD_params']['envmodules'].split("/")[1]
            oe_sor_upload(compact,
                          pro_type,
                          info_type,
                          desc,
            ...
            print(f"Software information for BD_analysis has been upload !Skipping!")
    if yaml_file["report_params"]["module"]["report"] == True:
        ...
        with open (i,'r') as f:
            for line in f :
                ...
                if line.startswith("rule "):
                    ll= re.split('\s+|:',line)
                    rule_name=ll[1]
                    threads=yaml_file['cpu']['cellranger']

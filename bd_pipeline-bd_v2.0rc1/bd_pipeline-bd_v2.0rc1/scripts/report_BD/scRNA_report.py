...
def scrna_report(program_path, configfile, rds):
		...
		if any([v!= 17 and v !=15 and v !=12  and v != 10 and v != 11 for k,v in file_number.items()]):
			...
			if len(sample_name) != Sample_Num:
				...
				for j in sample_name:
					...
					if os.path.exists(f"{outdir}/{module_num}.BD_Analysis/{j}/{j}.BAM" ):
						subprocess.call(f'rm {outdir}/{module_num}.BD_Analysis/{j}/{j}.BAM*', shell=True)
	...
	subprocess.call(f'rm -f {outdir}/{module_num}.supplemental_material/BD_单细胞转录组报告树状图.pdf', shell=True)
	subprocess.call(f'cp -r {scriptdir}/supplemental_material/BD_单细胞转录组报告树状图.pdf {outdir}/', shell=True)
...
# [^topGO]:Alexa A, Rahnenfuhrer J. topGO: enrichment analysis for gene ontology. R package version 2.8,2010.''')
	...
	link_Directory = 'BD_单细胞转录组报告树状图.pdf'
	...
	report_Directory_list.append(f'''[BD_单细胞转录组报告树状图]({link_Directory})\n''' )

...
find result/Diffexp/enrichment/ -name "*" -type f -size 0c |xargs -n 1 rm -f 
        """
# =============================================================================================
    ...
    run:
        ...
        if any(diff_stat["Total_diff(pvalue<0.05&FoldChange>1.5)"] < 20): 
            ...
            for diff_group in diff_groups.index :
                ...
                sig = all[(all["pvalue"] <0.05 )& ((all["FoldChange"]> 1.2  ) | (all["FoldChange"]< 1/1.2 ))]
...
Rscript {script_dir}/scVis \
  ...
  """)
                # ========== step3. ppi ========== 
                shell("module load {module} && python scripts/ppi/ppi_test.py -i {sig_file} -p {prefix} -o result/Diffexp/FC1.2/ppi -s {stringid} && Rscript  scripts/ppi/ppi_circle.r     -i {sig_network_file}    -d {sig_file}    -o result/Diffexp/FC1.2/ppi") 
...
-thread 2 -queue big && find result/Diffexp/FC1.2/enrichment/ -name "*" -type f -size 0c |xargs -n 1 rm -f """)

...
if type(config["BD_params"]["samples"]) == str :
    ...
    if config["BD_params"]["samples"] == "all" :
        ...
        if(config["BD_params"]["VDJ"] == "mRNA"):
            ...
            samples_tmp1 = list(set([item.replace('_dan', '') for item in samples]))
            samples_tmp = list(set([item.replace('_TCR', '') for item in samples_tmp1]))
...
def all_input_bd(wildcards):
    ...
    if config["BD_params"]["module"]["BD_upload"]:
        wanted_input.extend(
            [
                "BD_upload.check",
...
rule BD_Analysis:
    ...
    output:
        ...
        #temp_file=temp("result/BD_Analysis/{sample}temp")
    params:
        force_cells = get_FC
    benchmark:
...
#awk -F',' -v OFS='\t' '{print $14,$8}' {input.vdj_summary}|grep -A 3 "Chain_"|awk -F',' '{for(i=1;i<=NF;i=i+1){a[NR,i]=$i}}END{for(j=1;j<=NF;j++){str=a[1,j];for(i=2;i<=NR;i++){str=str " " a[i,j]}print str}}'|sed '1d'|awk '{print $2"\t"$3}'|
rule BD_sample_stat:
    input:
        web_summary="result/BD_Analysis/{sample}/{sample}_Pipeline_Report.html",
        metrics_summary="result/BD_Analysis/{sample}/{sample}_Metrics_Summary.csv"
...
############################upload force-cell result to auto analysis dir
if config["BD_params"]["module"]["BD_upload"]:
    #localrules: BD_upload
    rule BD_upload:
        """
        BD upload
        """
        input:
            web_summary=expand("result/BD_Analysis/{sample}/{sample}_Pipeline_Report.html",sample=samples),
            bam_log= expand("logs/{sample}.upload_bam_log.txt",sample=samples)
        output:
            check = "BD_upload.check"
        params:
            project = project,
        log:
            "logs/BD_upload.log"
        benchmark:
            "benchmarks/BD_upload.benchmark.txt"
        resources:
            qsub_mem=10,
            qsub_p=1
        shell:
            '''
bash scripts/upload_cellranger_m.sh $PWD/result/BD_Analysis {params.project} |& tee {log}  && touch {output.check}
            '''

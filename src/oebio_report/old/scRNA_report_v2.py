...
def scrna_report(input, configfile):
	...
	if os.path.exists("%s/Reference_celltype"  % (program_path)):
		...
		print("Can not find Reference_celltype results!!!!!!!!!!!!!!!!")
		log_file.write("Can not find Reference_celltype results!!!!!!!!!!!!!!!!" + "\n")
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
	...
	report.add_yaml_config('/home/luyao/oebio_report/old/report_v2.yaml')
	...
	#### vln_plots = sorted(list(glob('*.Marker/markers_vis4cluster'+i+'/*marker_gene_violin_plot.png' for i in range(12)  )))

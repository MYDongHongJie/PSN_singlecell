...
def scrna_report(input, configfile, rds):
	...
	if os.path.exists("%s/Clusters_correlation"  % (program_path)):
		...
		print("Can not find Clusters_correlation results!!!!!!!!!!!!!!!!")
		log_file.write("Can not find Clusters_correlation results!!!!!!!!!!!!!!!!" + "\n")
	...
	subprocess.call('cp -r /home/luyao/10X_scRNAseq_v3/src/oebio_report/supplemental_material %s/' % (outdir), shell=True)
	...
	# Define header info
	report = Report('10x单细胞转录组项目结题报告', title = '10x单细胞转录组项目结题报告', header_info = dict(config['par']), oe_welcome='''
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
	''')
	report.add_yaml_config('/home/luyao/10X_scRNAseq_v3/src/oebio_report/report.yaml')
	os.environ['OEBIO'] = "/home/luyao/10X_scRNAseq_v3/src/oebio_report/pic"
...
# [^topGO]:Alexa A, Rahnenfuhrer J. topGO: enrichment analysis for gene ontology. R package version 2.8,2010.''')
	...
	##########################loupe browser##########################
	link0 = 'supplemental_material/Loupe-Browser使用教程.html'
	supply_section_loupe = appendix_module.add_section('Loupe-Browser使用教程',link0=link0)

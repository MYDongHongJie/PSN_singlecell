...
def scrna_report(input, configfile):
	...
	# Define header info
	report = Report('BD单细胞转录组测序分析结题报告', title = 'BD单细胞转录组测序分析结题报告', header_info = dict(config['par']), oe_welcome='''
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
	''')
	report.add_yaml_config('/home/luyao/oebio_report/BD/report_BD.yaml')
	os.environ['OEBIO'] = "/home/luyao/oebio_report/BD/pic"
	...
	if sample_num == 1:        #1个样本
		if len(celltype_folder) == 0:        #特殊物种
			...
			linkage = project_module.add_section('项目快捷链接_无细胞类型_无差异',link_refdiff=link_refdiff, link0=link0)        #快捷链接
		else:
			...
			inkage = project_module.add_section('项目快捷链接_无差异',link_refdiff=link_refdiff, link0=link0)        #快捷链接
		...
		if len(celltype_folder) == 0:        #特殊物种
			...
			if len(Diffexp_folder) == 0:
				...
				linkage = project_module.add_section('项目快捷链接_无细胞类型_有差异',link_refdiff=link_refdiff, link0=link0)        #快捷链接
			...
			if len(Diffexp_folder) == 0:
				...
				linkage = project_module.add_section('项目快捷链接_无差异',link_refdiff=link_refdiff, link0=link0)        #快捷链接
				...
				linkage = project_module.add_section('项目快捷链接_差异',link_refdiff=link_refdiff, link0=link0)        #快捷链接
	...
	tsne_plots = list(glob('*.Clustering/tsne_groupby_cluster_resolution*_plot.png'))
	...
	Clustering_link = get_url(Clustering_folder,'tsne_Dimension_Reduction_coordination.csv')
	...
	if os.path.exists("%s/visualize_cluster_by_clusters" %(Clustering_folder)):
		...
		groupby_plot = groupby_module.add_plot(groupby_plots, caption = '样本间 t-SNE 分组展示图', description ='图片说明：横纵坐标分别代表t-SNE降维第一和第二主成分，不同样本来源的细胞以不同颜色区分。')
	...
	feature_plots = ["*.Marker/markers_vis4cluster"+str(i+1)+"/topmarker_gene_featureplot.png" for i in range(cluster_num) ]
	#### feature_plots = sorted(list(glob('*.Marker/markers_vis4cluster*/*marker_gene_featureplot.png')))
	...
	vln_plots = ["*.Marker/markers_vis4cluster"+str(i+1)+"/topmarker_gene_violin_plot.png" for i in range(cluster_num) ]
	#### vln_plots = sorted(list(glob('*. /markers_vis4cluster'+i+'/*marker_gene_violin_plot.png' for i in range(12)  )))

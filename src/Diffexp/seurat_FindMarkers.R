...
option_list <- list(
    ...
    make_option(c("--clusterids"), default="none",
                help="The column name of cluster identities fiels."),
    ...
    make_option(c("--annotation"), default="not_set",
                help=paste('A gzipped file with "ensembl_id" and "gene_name" columns.',
                           'Used if seurat_ob@misc$gene_id is missing')),
...
seurat_ob <- readRDS(opt$seuratobject)
#cluster_ids <- readRDS(opt$clusterids)
cluster_ids <- seurat_ob$meta.data[,opt$clusterids])

have_gene_ids <- "gene_id" %in% colnames(seurat_ob@misc)
...
if(!identical(seurat_ob@cell.names,names(cluster_ids))) {   
    stop("Cluster cell names do not match Seurat object cell names")
}

if(!identical(names(cluster_ids), rownames(seurat_ob@meta.data))) {
...
if(opt$testfactor=="none") {
    ...
    if(!opt$a %in% seurat_ob@meta.data[[opt$testfactor]] | !opt$b %in% seurat_ob@meta.data[[opt$testfactor]]) {
        stop("between_a and/or between_b not present in the opt$testfactor metadata colum")
    }
    ident[cluster_ids==id & seurat_ob@meta.data[[opt$testfactor]] == opt$a] <- "a"
    ident[cluster_ids==id & seurat_ob@meta.data[[opt$testfactor]] == opt$b] <- "b"
}

if(opt$conservedfactor != "none"){
    # stopifnot testfactor also supplied
    ident.conserved <- factor(seurat_ob@meta.data[names(ident), opt$conservedfactor])
...
for (conserved.level in levels(ident.conserved)){
    ...
    seurat_ob@ident <- factor(ident.use)
    ident.use = seurat_ob@ident
    message("Identities:")
    print(table(seurat_ob@ident))
    ...
    if(length(unique(seurat_ob@ident[seurat_ob@ident!="ignore"])) > 1) {
        nclust_check = TRUE

        cluster_cells <- seurat_ob@cell.names[seurat_ob@ident == "a"]
        other_cells <- seurat_ob@cell.names[seurat_ob@ident == "b"]
        ...
        genes <- rownames(seurat_ob@data)
        cluster_pct <- apply(seurat_ob@data[genes,cluster_cells, drop=F], 1, function(x) round(sum(x>0)/length(x), digits=3))
        other_pct <- apply(seurat_ob@data[,other_cells, drop=F], 1, function(x) round(sum(x>0)/length(x),digits=3))
        ...
        cluster_mean <- apply(seurat_ob@data[,cluster_cells, drop=F], 1, FUN = ExpMean)
        other_mean <- apply(seurat_ob@data[,other_cells, drop=F], 1, FUN = ExpMean)
        ...
        filter_stats <- data.frame(row.names=rownames(seurat_ob@data),
        ...
        genes.use <- rownames(seurat_ob@data)[take]
        background.use <- rownames(seurat_ob@data)[background_take]
    ...
    if(nclust_check & ncells_check & other_checks) {
        ...
        genes.de <- FindMarkers(seurat_ob, ident.1 = "a", ident.2 = "b",
                                genes.use = NULL, logfc.threshold = opt$threshuse,
                                test.use = opt$testuse, min.pct = opt$minpct,
                                min.diff.pct = opt$mindiffpct, print.bar = F,
                                min.cells.gene = min.cells, min.cells.group = min.cells)
        ...
        if(have_gene_ids) {
            ...
            markers$gene_id <- seurat_ob@misc[markers$gene,"gene_id"]
        } else {
        ...
        write.table(markers, gzfile(out_path),
        ...
        if(have_gene_ids) {
            ...
            universe_gene_ids = seurat_ob@misc[background.use,"gene_id"]
        } else {
...
if (length(markers.conserved.list) > 1){
    ...
    if(have_gene_ids) {
        print("Writing universe gene ids using @misc$gene_id")
        universe_gene_ids = seurat_ob@misc[background.conserved.genes,"gene_id"]
    } else {
        print("Writing universe gene ids using annotation file")
        ## avoid writing out NA values.

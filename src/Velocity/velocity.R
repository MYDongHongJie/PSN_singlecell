...
option_list <- list(
    ...
    make_option(c("--minmaxclustavemat"), type="double", default=0.2,
                help="min.max.cluster.average for exon matrix filtering"),
    make_option(c("--minmaxclustavnmat"), type="double", default=0.05,
                help="min.max.cluster.average for intron matrix filtering"),
    make_option(c("--deltat"), type="double", default=1, help="deltaT parameter"),
    make_option(c("--kcells"), type="integer", default=15,
                help="number of cells for knn graph used for velocity estimates"),
    make_option(c("--fitquantile"), type="double", default=0.02, help="fit.quantile parameter"),
    make_option(c("--neighbourhoodsize"), type="integer", default=100,
                help="neighbourhood size"),
    make_option(c("--velocityscale"), type="character", default="log",
                help="velocity scale to use (log, sqrt, rank or linear)"),
    make_option(c("--arrowscale"), type="double", default=3,
                help="arrow scale multiplier"),
    make_option(c("--arrowlwd"), type="double", default=1.25, help="arrow line weight"),
    make_option(c("--gridflow"), type="logical", default=TRUE,
                help="show grid velocity summary"),
    make_option(c("--mingridcellmass"), type="double", default=0.5,
                help="min.grid.cell.mass parameter"),
    make_option(c("--gridn"), type="integer", default=40,
                help="number of grid points along each axis"),
    make_option(c("--cellalpha"), type="double", default=0.8,
                help="alpha value for cell border"),
    make_option(c("--cellborderalpha"), type="double", default=0,
                help="alpha value for cell border"),
    make_option(c("--showaxes"), type="logical", default=TRUE, help="show plot axes"),
    make_option(c("--plotdirvar"), default="velocityDir",
                help="latex var holding location of plots"),
    make_option(c("--plotcex"), type="double", default=1,
                help="overall cex for the plot"),
    make_option(c("--ncores"), type="integer", default=12,
                help="number of cores for velocity estimation"),
    make_option(c("--outdir"), default="seurat.out.dir", help="outdir")
    )
opt <- parse_args(OptionParser(option_list=option_list))

## read in the reduced dimension coordinates
rdims <- read.table(opt$rdimstable, header=T, sep="\t",as.is=T)
...
emat <- dat[grepl("ex_",rownames(dat)), colnames(dat) %in% rdims$barcode,drop=F]
nmat <- dat[grepl("in_",rownames(dat)), colnames(dat) %in% rdims$barcode,drop=F]

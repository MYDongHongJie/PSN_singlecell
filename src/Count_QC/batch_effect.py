...
@click.command()
@click.option('-i','--input', prompt='input directory',
              help='e.g. filtered_gene_bc_matrices')
@click.option('-l','--log', default="no", prompt='Log standardization',
             help='log standardization')
@click.option('-o','--outdir', prompt='output dir',
             help='your working directory')

def batch_effect(input,outdir,log):
    """python batch_effect.py -i /public/cluster2/works/luyao/test/scRNA/filtered_gene_bc_matrices -l no -o /public/cluster2/works/luyao/test/scRNA"""

    subprocess.check_call('mkdir -p %s/batch_effect' % (outdir), shell=True)
    subprocess.check_call('/home/hanmin/software/cellranger-2.1.1/cellranger-cs/2.1.1/bin/cellranger mat2csv %s  %s/matrix.csv' % (input, outdir), shell=True)
    
    with open('%s/matrix.csv' %(outdir)) as f1:
        fr1 = f1.readlines()[1:]
    
    genomes=os.listdir(input)
    for genome in genomes:
        matrix_dir = os.path.join(input, genome)
        ...
        with open('%s/express.txt' %(outdir),'w') as f2:
            f2.write('CELL'+'\tCELL'.join([str(i) for i in range(1,len(barcodes)+1)])+'\n')
            f2.write(''.join(fr1).replace(',','\t'))
    ...
    pheno = pd.read_table('%s/pheno.txt' %(outdir), index_col=0)
    dat = pd.read_table('%s/express.txt' %(outdir), index_col=0)
    if log == "no":
        dat = np.log2(dat+1.1).round(2)
    
    #pheno = pd.read_table('/public/cluster2/works/luyao/test/scRNA/combat/bladder-pheno.txt' , index_col=0)
    #dat = pd.read_table('/public/cluster2/works/luyao/test/scRNA/combat/bladder-expr.txt' , index_col=0)
    #mod = patsy.dmatrix("~ age", pheno, return_type="dataframe")
    t = time.time()
    ebat = combat(dat, pheno['batch'])
    sys.stdout.write("%.2f seconds\n" % (time.time() - t))

    ebat.to_csv("%s/batch_effect/batch_express_log.txt"  %(outdir), sep="\t")

    #ebat = combat(dat, pheno['batch'], None)
    
    
    dat2 = pd.read_table('%s/batch_effect/batch_express_log.txt' %(outdir), index_col=0)
    dat2 = np.power(2,dat2)-1.1
    dat2 = dat2.round(2)
    dat2.to_csv("%s/batch_effect/batch_express.txt" %(outdir),sep="\t")
    os.remove("%s/batch_effect/batch_express_log.txt"  %(outdir))

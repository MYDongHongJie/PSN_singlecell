...
...
    ...
    object = switch (tolower(reduct2.use),
                ...
                'svd' = RunSVD( object, n = npcs.use, features = feature.use,
                                verbose = F, ...),
                'tsne' = RunTSNE(object, reduction = reduct1.use,seed.use=1,
                ...
                'harmony' = RunHarmony(object, group.by.var = batch.use, dims.use = 1:npcs.use,
                                max.iter.cluster = 20, ...),
                ...
                'umap' = RunUMAP(object,dims = 1:npcs.use,verbose = F,seed.use=1,
    ...
    message(paste0(reduct2.use, " Dimension Reduction finished npcs.use:",npcs.use))
    return( object )
}
...
option_list = list(
    ...
    make_option( c("--component_pca", "-y"), type="integer", default=NULL,
        help="The value of the npcs used for PCA."),
    ...
    make_option( c("--integrated", "-e" ), type="character", default = FALSE,
        help="integrated 样本整合方法！"),
    make_option( c("--kweight", "-w" ), type="integer", default = 100,
...
if ( !is.null(opt$informat) & !is.null(opt$input) ){
    if ( opt$informat == "seurat" ){
		...
		if ('rawbc' %in% colnames(singlecell_ob@meta.data)) {
        ...
        if ( as.logical(opt$integrated) ){
            ...
            # 按样本batchid进行拆分，后续可以改
            #singlecell_ob = Seurat::SetIdent( singlecell_ob, value = "sampleid")
            meta_features = singlecell_ob@assays$RNA@meta.features
            ifnb.list <- SplitObject(singlecell_ob, split.by = opt$batchid)
            ...
            singlecell_ob <- IntegrateData(anchorset = immune.anchors,k.weight = opt$kweight)
            #指定我们将对校正后的数据执行下游分析注意原始未修饰数据仍驻留在“RNA”测定中
            Seurat::DefaultAssay(singlecell_ob) <- "integrated"
            singlecell_ob <- ScaleData(singlecell_ob, verbose = FALSE)
            singlecell_ob@assays$RNA@var.features <- singlecell_ob@assays$integrated@var.features
            singlecell_ob@assays$RNA@scale.data <- singlecell_ob@assays$integrated@scale.data
            singlecell_ob@assays$RNA@meta.features <- meta_features
            #singlecell_ob = immune.combined
            # 获取高变，用于pca等等
            #variablefs = Seurat::VariableFeatures(singlecell_ob)
            }       
#======Determine statistically significant principal components for clustering=====
reduct1s <- as.character(unlist(stringr::str_split(reduct1, ",")))
...
if ( reduct1 %in% names(Key(singlecell_ob)) & opt$rerun == F ){#if previous reduction has been calculated ,donot rerun
    ...
    singlecell_ob = RunDimReduc(singlecell_ob, reduct1.use = ifelse(
                        reduct1 == "harmony",
                        yes = "pca",
                        no = reduct1
                    ),
                        ...
                        batch.use = batchid, npcs.use = ifelse(
                        reduct1 == "pca",
                        yes = component_pca,
                        no = opt$components
                    ) )
    reduct1_coord = FetchData(singlecell_ob,
                                var = c(Barcode_content, paste0( Key(singlecell_ob)[reduct1], 1:2))) %>%
...
if ( !is.null( reduct2) & tolower(reduct2) != "null"){
    ...
    reduct2_coord = FetchData(singlecell_ob,
                        var = c(Barcode_content, paste0( Key(singlecell_ob)[reduct2], 1:2))) %>%
                        dplyr::rename( "Barcode" = Barcode_content)
...
ggdimplot = DimPlot(object = singlecell_ob,reduction = reduct2, dims = c(1,2),
             label = T,  pt.size = pointsize, group.by= cluster_result_colname)

ggdimplot = ggdimplot + scale_colour_manual(values = CustomCol2(1:length(unique(Idents(singlecell_ob)))),
                                            breaks = levels(Idents(singlecell_ob)),
                                            labels = cell_count_labels)+
                        guides(colour = guide_legend(ncol = ifelse(length(unique(Idents(singlecell_ob))) <= 20,1,2),
						     override.aes = list(size = 2.5)))
ggsave(file.path(output_dir,paste0(reduct2, "_groupby_cluster","_resolution",resolution,"_plot.pdf",collapse="")),
       plot = ggdimplot,width = ifelse(length(unique(Idents(singlecell_ob))) <= 20,7.5,9))
ggsave(file.path(output_dir,paste0(reduct2, "_groupby_cluster","_resolution",resolution,"_plot.png",collapse="")),dpi=1000,
       plot = ggdimplot,width = ifelse(length(unique(Idents(singlecell_ob))) <= 20,7.5,9))

if ( as.logical(opt$integrated) ){
  print("修改DefaultAssay")
  Seurat::DefaultAssay(singlecell_ob) <- "RNA"
}

tsne_clusters_info = singlecell_ob@meta.data %>%
						 dplyr::rename( "Barcode" = Barcode_content) %>%
						 select( Barcode, sampleid, clusters, group )
write.table(tsne_clusters_info, quote = F,
			file.path(output_dir,paste0(reduct2,"_clusters_results.csv",collapse="")), sep =",",row.names =F)

saveRDSMC(singlecell_ob, file.path(dirname(output_dir), paste0("singlecell_object.clustering_resolution", resolution, ".rds", collapse="")) )

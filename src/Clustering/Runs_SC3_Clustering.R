...
### 'data.table' to speed up reading matrices with 'fread' instead of 'read.table'
### 'SingleCellExperiment' (bioconductor) to create SingleCellExperiment objects
### 'SC3'    (bioconductor) to do the SC3 clustering
### 'scater' (bioconductor) to generate scatter plots and calculate single-cell QC metrics
####################################
...
if (InfileCellTypes == "NA") {
  singlecellexp.object <- SingleCellExperiment(assays = list(counts = as.matrix(fullmat), 
					logcounts = log2(as.matrix(fullmat) + AddPseudocount)))
}else{
  singlecellexp.object <- SingleCellExperiment(assays = list(counts = as.matrix(fullmat), 
					logcounts = log2(as.matrix(fullmat) + AddPseudocount)), colData = celltypes)
}
...
sapply(outfiles_to_move,FUN=function(eachFile){
  file.rename(from=paste(Tempdir,"/",eachFile,sep=""),to=paste(Outdir,"/SC3/",eachFile,sep=""))
})

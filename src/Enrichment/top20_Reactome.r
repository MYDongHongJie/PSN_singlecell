...
opt_parser = OptionParser(option_list=option_list,epilogue = "Rscript top20_Reactome.r -i enrichment-Reactome-Group1-vs-Group2-Down.txt -m Down -o outdir/");
...
top20 <- head(enrich[order(enrich[,"pval"]),], 20)
...
write.table(top20[,c(1,2,7,9,10)], paste0(opt$outpath, "/Reactome.top.", opt$mark, ".xls"), sep="\t",
	quote=FALSE, col.names=TRUE, row.names=FALSE)

top20[, "term"] <- sub("^path:", "", paste(top20[,1], top20[,2], sep=":"))

xlab <- "Enrichment Score"
title <- paste0(groupname, ": ", "Reactome Enrichment top 20")
size.lab <- "Number"
p=qplot(Enrichment_score, term, data=top20, size=ListHits, 
	colour=pval, xlab=xlab, ylab="")+ggtitle(title)+
	theme_bw()+
	theme(plot.title = element_text(hjust = 0.5))+
	scale_colour_gradientn(colours=rainbow(6)) +
	labs(size=size.lab) + labs(colour="pvalue")
ggsave(paste0(opt$outpath, "/Reactome.top.", opt$mark, ".pdf"), height=8, width=10, plot=p)
ggsave(paste0(opt$outpath, "/Reactome.top.", opt$mark, ".png"), type="cairo-png", height=8, width=10, plot=p)

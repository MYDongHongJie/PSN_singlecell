...
######### GENERATING CSV Files starts here
    ...
    parser.add_argument('-i','--input', help='inputfile', required=True, type=str, default='Inputs')
    parser.add_argument('-o','--output', help='outputfile', required=False, type=str, default='CNV_files')
    ...
    for working_file in os.listdir(Inputs):
        if working_file.endswith(".cell_groupings"):
            nr_of_inputs += 1
            with open(os.path.join(Inputs ,working_file)) as groupings_file:
        ...
        #### determine percentages and remove cells with cutoff
            ...
            with open(os.path.join(output_dir,working_file+'.csv'), mode='w') as output:
...
#colors = ["#8E8E8E", "#EA9898", "#71C2C6", "#EDB843", "#C270E8", "#6E5BDD", "#7FAD7F", "#8E8E8E", "#EA9898", "#71C2C6", "#EDB843", "#C270E8", "#6E5BDD", "#7FAD7F", "#8E8E8E", "#EA9898", "#71C2C6", "#EDB843", "#C270E8", "#6E5BDD", "#7FAD7F", "#8E8E8E", "#EA9898", "#71C2C6", "#EDB843", "#C270E8", "#6E5BDD", "#7FAD7F"]  # first is grey
    ...
    filename = os.path.join(output_dir, "output.svg")
    ...
    # 将SVG文件转换为PNG格式
        png_data = cairosvg.svg2png(url=svg_file, dpi=1000, parent_width =700,parent_height=700)
    # 使用PIL库打开PNG文件
    ...
    plot_nr = -1
    for working_file in os.listdir(output_dir):
            if working_file.endswith(".csv"):
                ...
                with open(os.path.join(output_dir,working_file)) as current_file:
    ...
    write_to_file('''<svg viewBox="0 0 ''' + str(nr_of_inputs*150) + ''' 300" xmlns="http://www.w3.org/2000/svg" style="background-color: #ffffff;">''')
...
##############  Replace all CSV files, and add which letter was assigned to the clones
    ...
    for working_file in os.listdir(output_dir):
        if working_file.endswith(".csv"):
            ...
            with open(os.path.join(output_dir,working_file), mode='w') as output:
                writer = csv.writer(output, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                for i in input_data:
                    writer.writerow(i)
    svg_to_white_bg_jpg(svg_file=filename, output_jpg_file=os.path.join(output_dir,'output.png'))

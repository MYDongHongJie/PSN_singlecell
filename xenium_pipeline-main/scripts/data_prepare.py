...
def download(sub_addresses,outdir = "raw_data"): # sub_addresses in a list
    for sub_address in sub_addresses:
        subprocess.run(
            f"/data/software/obsutil/5.2.12/obsutil cp {sub_address}  {outdir} -r -f -vlength", shell=True, check=True)
...
def raw_data_download(addresses,sample_list,outdir):
    ...
    for sample in sample_list :
        print(sample)
        if os.path.exists(f'{outdir}/{sample}/cell_feature_matrix'):
    ...
    if len(sample_list2download) >0:
        ...
        for address in addresses:
            ...
            batch += 1
## rename data =========================================================================================================
# def raw_data_formart(sample_list,outdir,xenium_method):
#     for sample in sample_list:
...
def run(configfile,outdir):
    ...
    addresses = config["params"]["Xenium"]['raw_data_obs_address']
    ...
    #raw_data_formart(sample_list,outdir,xenium_method)

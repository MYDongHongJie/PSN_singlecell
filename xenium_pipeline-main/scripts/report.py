...
def multimodal_report(inputdir, configfile, dirnum):
    ...
    if os.path.exists("%s/3.Clustering" % (program_path)):
        ...
        subprocess.call('rm -rf %s/3.Clustering/*.rds' % (outdir), shell=True)
        subprocess.call('rm -rf %s/3.Clustering/visualize_cluster_by_clusters' % (outdir), shell=True)
    else:
        print("Can not find 3.Clustering results!!!!!!!!!!!!!!!!")
        log_file.write("Can not find 3.Clustering results!!!!!!!!!!!!!!!!!" + "\n")

    # 4.Visualize_cluster_by_clusters
    if os.path.exists("%s/3.Clustering/visualize_clusters_by_clusters" % (program_path)):
        os.makedirs("%s/4.Visualize_clusters_by_clusters" % (outdir))
        subprocess.call('cp -r %s/3.Clustering/visualize_clusters_by_clusters/* %s/4.Visualize_clusters_by_clusters' % (program_path, outdir), shell=True)
    else:
        print("Can not find 4.Visualize_clusters_by_clusters results!!!!!!!!!!!!!!!!")
        log_file.write("Can not find 4.Visualize_clusters_by_clusters results!!!!!!!!!!!!!!!!!" + "\n")
    ...
    if os.path.exists("%s/6.Reference_Celltype" % (program_path)):
        ...
        subprocess.call('rm -rf %s/6.Reference_Celltype/*.rds' % (outdir), shell=True)
    else:
        print("Can not find 6.Reference_Celltype results!!!!!!!!!!!!!!!!")
        log_file.write("Can not find 6.Reference_Celltype results!!!!!!!!!!!!!!!!!" + "\n")

    # 7.Visualize_celltype_by_celltype
    if os.path.exists("%s/6.Reference_Celltype/visualize_celltype_by_celltype" % (program_path)):
        os.makedirs("%s/7.Visualize_celltype_by_celltype" % (outdir))
        subprocess.call('cp -r %s/6.Reference_Celltype/visualize_celltype_by_celltype/* %s/7.Visualize_celltype_by_celltype' % (program_path, outdir), shell=True)
    else:
        print("Can not find 7.Visualize_celltype_by_celltype results!!!!!!!!!!!!!!!!")
        log_file.write("Can not find 7.Visualize_celltype_by_celltype results!!!!!!!!!!!!!!!!!" + "\n")
    ...
    header_info["物种信息"] = header_info.pop("Species").capitalize()
    ...
    report = Report('10x Xenium 组织原位分析项目结题报告', title='10x Xenium 组织原位分析项目结题报告',
    ...
    if os.path.exists("2.Count_QC"):
        ...
        if len(summ) == 1:
            report_summary_list.append(
                f"本次分析共完成 {sample_num} 个样本的10x Xenium 组织原位分析，数据经Xenium Ranger重新运行，用10x最新算法分割细胞，最终获得{max_cell}个细胞，每个细胞中的平均UMI数为{max_nCount}，平均基因数为{max_nFeature}。")
        else:
            report_summary_list.append(
                f"本次分析共完成 {sample_num} 个样本的10x Xenium 组织原位分析，数据经Xenium Ranger重新运行，用10x最新算法分割细胞，最终获得的细胞数目分布在{min_cell}~{max_cell}个，每个细胞中的平均UMI数分布在{min_nCount}~{max_nCount}，平均基因数分布在{min_nFeature}~{max_nFeature}。")
        ###
        filter_section.add_table(summ,caption='样本质控情况统计表')
        filter_section.add_section('基因表达数目统计')
        filter_section.add_section('转录本数目统计')
    ##====================================================== 4.3 降维聚类 =================================================
    if os.path.exists("3.Clustering"):
        ...
        if config['params']['Clustering']["reduct1"] == "pca":
            ...
            reduct1_name = "MNN"
        if config['params']['Clustering']["reduct2"] == "tsne":
        ...
        ## 样本分组展示
    ##===================================================== 4.4 降维聚类可视化 =================================================
    if os.path.exists("4.Visualize_clusters_by_clusters"):
        if (sample_num > 1):
    ...
    ##===================================================== 4.4 细胞类型分面可视化 =================================================
    if os.path.exists("7.Visualize_celltype_by_celltype"):
        if (sample_num > 1):
            celltype_section =  rna_section5.add_section('细胞类型分组展示')
            report_hyperlink_list.append('[细胞类型分组展示](#celltype_vis)\n\n')
            celltype_section.add_section('细胞类型分样本展示图')
            celltype_section.add_section('细胞类型分样本占比统计图')
    ...
    database = pd.DataFrame(
        [['Panel Database', 'https://www.10xgenomics.com/support/in-situ-gene-expression/documentation/steps/panel-design/pre-designed-xenium-gene-expression-panels'],

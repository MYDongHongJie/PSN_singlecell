...
# }
#=======================================================================================================================
statisc_summary<- function(xenium,samplelist, outdir){
    lapply(samplelist, function(sample) {
        html<-sprintf("%s/%s/outs/analysis_summary.html", xenium, as.character(sample))
        csv<-sprintf("%s/%s/outs/metrics_summary.csv", xenium, as.character(sample))
    ...
    write.table(summary, file=paste0(outdir, "/", "summary_all.csv"), sep=",",quote = FALSE, col.names=F,na="")
    write.table(as.data.frame(t(summary)), file=paste0(xenium, "/", "summary_all.csv"), sep=",",quote = FALSE, row.names=F,col.names=T,na="")
}
#=======================================================================================================================
...
parser$add_argument("-i", "--xenium", default="./result/Xenium", help="spaceranger output directory[default \"%(default)s\"]")
...
outdir <- paste0(args$outdir, "/", project_id, "-QC_report")
...
summary_anno <- statisc_summary(args$xenium,samplelist, outdir)

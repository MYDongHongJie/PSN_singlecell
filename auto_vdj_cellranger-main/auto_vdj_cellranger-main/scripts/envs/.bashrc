...
# smq () {
        # export TMP=./
        # /data2/software/miniconda/envs/snakemake/bin/snakemake  \
             # --cluster "scrnasub"  \
...
#======================= snakemake test
smt () {
/public/scrna/software/miniconda3/envs/snakemake/bin/snakemake  -npr -s $1
 }

#====================== snakemake plot
smpl () {
/public/scrna/software/miniconda3/envs/snakemake/bin/snakemake  -npr -s $1 --dag| dot -T$2 > $3
 }

#====================== snakemake unlock
smu(){
if [[ $# -eq 0 ]]; then /public/scrna/software/miniconda3/envs/snakemake/bin/snakemake  --unlock
else /public/scrna/software/miniconda3/envs/snakemake/bin/snakemake  --unlock -s $1
fi
}

smq(){
export TMP=./ && /data2/software/miniconda/envs/snakemake/bin/snakemake  --cluster "scrnasub" --cluster-config config/cluster_txy.yaml -j 10 --latency-wait 3000 -s $1  >&2 2>> run_snakejob.$(date +%Y%m%d%H%M%S).log 
}
# alias smq="export TMP=./ && nohup /data2/software/miniconda/envs/snakemake/bin/snakemake  --cluster "scrnasub" --cluster-config config/cluster_txy.yaml -j 10 --latency-wait 3000 -s cellranger.smk  >run_snakejob.$(date +%Y%m%d%H%M%S).log &"
alias qstat='func() { kubectl get pods -n snakemake-scrna;}; func'
alias geti='func() { kubectl logs -n snakemake-scrna `kubectl get pods -n snakemake-scrna | grep $1 | cut -f 1 -d " "`;}; func'

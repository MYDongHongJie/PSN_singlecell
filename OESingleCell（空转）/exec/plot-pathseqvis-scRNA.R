    ...
    output_dir)
{
    ...
    pdf(NULL)
    plot_featureplot <-Seurat::FeaturePlot(data_ob_sub,
                                           features = features,
                                           #cols="spectral",
                                           min.cutoff = 0,
                                           ##max.cutoff ="q100",
     ...
     width<-4.5*(1+legend_length)
     plot_legend <- ggpubr::get_legend( plot_featureplot) %>%ggpubr::as_ggplot()
     plot<-patchwork::wrap_plots(plot_featureplot+Seurat::NoLegend(),
                                 plot_legend,
                                 nrow = 1,
                                 ncol = 2,
                                 widths=c(1,legend_length))
    OESingleCell::save_ggplots(plot=plot,
                               filename =glue::glue("{output_dir}/featureplot_{name}_for_{sample}"),
                               width= width,
                               height=4.5)
...
for(sample in sample_list){
 ...
 suppressMessages(result_genus_raw  <-do.call(dplyr::bind_rows, lapply(sample_list, function(sample)
                readr::read_csv(glue::glue('{input_dir}/{sample}.visium.genus.csv'))%>%
                                     ...
                                     dplyr::mutate(barcode=paste0(sample,"_",a),sampleid=sample)%>%
                                     dplyr::select(!c("a","b"))%>%
                                     # replace(is.na(.), 0)  %>%
 ...
 futile.logger::flog.info("获取barcode X genus 矩阵，存入misc ,然后统计每个spot中Bacteria_UMIs，并导入meta.data")
    ...
    colnames(result_genus)<- gsub("\\)","",colnames(result_genus) )
    colnames(result_genus)<- gsub("\\-","_",colnames(result_genus) )
#    colnames(result_genus)<- gsub("\\)","",colnames(result_genus) )
...
suppressMessages(result_reads<-do.call(rbind, lapply(sample_list, function(sample)
               readr::read_tsv( glue::glue('{input_dir}/{sample}.visium.filtered.readnamepath'))%>%
                        ...
                        tidyr::separate(barcode,sep="-",into=c("a","b")) %>%
                        dplyr::mutate(barcode=paste0(sample,"_",a))%>%
                        dplyr::select(!c("a","b"))%>%
                        tibble::column_to_rownames("barcode"))))
...
sub_pathseq <- subparsers$add_parser("pathseqvis_scrna",
...
if ("pathseqvis_scrna" %in% args) {
    opt <- intial_setting()
    if (opt$sub_name == "pathseqvis_scrna") {
        # ===================================================================================================================
        ...
        # ===================================================================================================================
        futile.logger::flog.info("step3.绘制各个spot中的Bacteria_reads、Bacteria_UMIs的featureplot分布图及小提琴图")
        futile.logger::flog.info("绘制featureplot图")
        sample_list<-list.files(path = input_dir, pattern ="visium.raw.readnamepath") %>%
                 stringr::str_replace_all(pattern = '.visium.raw.readnamepath',replacement = "")
        parallel::mclapply(sample_list, function(sample) {

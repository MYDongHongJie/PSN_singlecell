...
check_and_email<- function(summary_csv){
    ...
    if ( all(summary_stat["Number of Reads",] >= as.integer(raw_reads)*1e6*0.9) &&
        ...
        if (report) {
            ...
            <p>各样本spaceranger运行质控结果文件详见附件，请查收。</p> 
            <p>该项目为只质控不分析项目，已上传 QC 云报告。</p> 
            <p>路径：obs://oe-scrna/Analysis_Report/{project_number}/{report_name}。 </p> ')) # 邮件正文
        }else{
            body <- as.character(glue::glue('<p> 朱卉，</p>
            <p>您好，{customer}老师的空间转录组项目{project_id} {sample_number}个{species}样本（{sample_list})质控已完成。</p>
            <p>质控结果显示{sample_list}等{sample_number}个样本的部分指标存在异常，详细情况见下表说明，请知悉。</p>
   ...
   ####
    ...
    if (report){
        subject <- as.character(glue::glue("【质控反馈和项目报告】{project_id} {customer}老师-Visium 项目质控反馈和项目报告"))  #主题
    }else{
        subject <- as.character(glue::glue("【质控反馈】{project_id} {customer}老师-Visium 项目质控反馈"))  #主题
    }
...
")
        ...
        send.mail(from="sc_autosend@oebiotech.com",
                to= to,
                subject=subject,
                body=merge_body,
                encoding = "utf-8",
                ...
                smtp = list(host.name = "smtp.oebiotech.com",  # 此处以QQ邮箱为例
                            ...
                            user.name = "sc_autosend@oebiotech.com" ,
                            passwd = "OEbiosc1616" ,
                            ssl = TRUE),
                authenticate = TRUE,
                send = TRUE,
...
statisc_summary<- function(spaceranger, samplelist, outdir){
    lapply(samplelist, function(sample) {
        ...
        file.copy(from = csv, to = paste0(outdir, "/",as.character(sample), "_", "metrics_summary.csv"))
    })

    summarylist<-list.files(path = outdir, pattern = "_metrics_summary.csv",full.names =TRUE)
    summary<- do.call(cbind, lapply(summarylist,function(i) { t(read_csv(i, col_names = TRUE)%>% mutate_if(is.numeric, round, 4))}))
    print(summary)
    write.table(summary,
    file=paste0(outdir, "/", "summary.csv"),
    sep=",",quote = FALSE,
    col.names=F)
}
#=======================================================================================================================
...
docstring <- " example1:\\n\\n\\
sctool    -o results   st-email  -s -i -c "
...
sub_email$add_argument(
"-i", "--spacernager", default="result/spaceranger", help="spaceranger output directory[default \"%(default)s\"]")
...
if ("st-email" %in% args) {
    opt <- intial_setting()
    if (opt$sub_name == "st-email") {
        #=======================================================================================================================
        ##step1  config.yaml parse
        config<-yaml.load_file(opt$config)
        project_id<-config$report$Task_Num
        #project_number <-  gsub("-.*","", project_id)
        project_type<-config$report$Project_Type
        Project_End_Time<-config$report$Project_End_Time
        if (Project_End_Time==""){
            Project_End_Time=Sys.Date()
        }
        report_name <- paste0(project_id,"_QC_Report_",Project_End_Time,".zip" )
        customer<-config$report$Customer
        species<-config$report$Species
        sample_number<-config$report$Sample_Num
        raw_reads<-config$params$raw_reads
        library_type<-config$params$library_type
        report <- config$module$spaceranger_report
        if (config$params$email_to==""){
            if (Sys.getenv("CLOUDUSER") == "") {
                print("There's no CLOUDUSER variable in the current environment, please set CLOUDUSER on .bashrc or email to on config.yaml ")
            }else{
                email_to<- Sys.getenv("CLOUDUSER")
            }
        }else{
            email_to<-config$params$email_to
        }
        #=======================================================================================================================
        ###step2 outdir check and make
        outdir <- paste0(output_dir, "/", project_id, "-QC_spaceranger_report")
        ...
        statisc_summary(opt$spacernager, samplelist, outdir)

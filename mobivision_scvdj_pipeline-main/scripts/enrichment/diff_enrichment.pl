...
system("awk -F \"\\t\" 'NR==1 {for (f=1; f <= NF; f+=1) {  if (\$f ~ /Regulation/) flag=f } ;print \$0 } ;{if(\$flag ==\"Down\") print \$0} ' $diff_infile > $outdir/$name-Down.xls");
system("awk -F \"\\t\" 'NR==1 {for (f=1; f <= NF; f+=1) {  if (\$f ~ /Regulation/) flag=f } ;print \$0 } ;{if(\$flag ==\"Up\") print \$0} ' $diff_infile > $outdir/$name-Up.xls");
...
system("$env && Rscript $Bin/enrichment_clusterprofiler.r -i $outdir/$name-Down.xls,$outdir/$name-Total.xls,$outdir/$name-Up.xls -j $go_bg -c $category -k $kegg_bg  -p $name-Down,$name-Total,$name-Up  -o $outdir -n Gene -m 5 ");
print "$env && Rscript $Bin/enrichment_clusterprofiler.r -i $outdir/$name-Down.xls,$outdir/$name-Total.xls,$outdir/$name-Up.xls -j $go_bg -c $category -k $kegg_bg  -p $name-Down,$name-Total,$name-Up  -o $outdir -n Gene -m 5 ";
...
system("rm $outdir/$name-Up.xls ");
system("rm $outdir/$name-Total.xls ");
system("rm $outdir/$name-Down.xls ");

...
###################### excute ####################
if (all(file.exists(Sys.glob(file.path(vdj_path,rownames(assay_metadata)))) )){
  # # glob all the clonotype annotation file for all the samples
  clonotypes_tbs = file.path(vdj_path,rownames(assay_metadata),"outs",paste0(rownames(assay_metadata),"_","clonotypes.csv"))
  names(clonotypes_tbs) = paste0(assay_metadata$sampleid,"_",type)
  filtered_contig_tbs = file.path(vdj_path,rownames(assay_metadata),"outs",paste0(rownames(assay_metadata),"_","filtered_contig_annotations.csv"))
  names(filtered_contig_tbs) = paste0(assay_metadata$sampleid,"_",type)
  vdj_gene_location = file.path(vdj_path,rownames(assay_metadata),"outs",paste0(rownames(assay_metadata),"_","airr_rearrangement.tsv"))
  names(vdj_gene_location) = paste0(assay_metadata$sampleid,"_",type)
}else{
  stop("NO mobivision VDJ output directory available! Please make sure the samples' ID in the metadata are the same as
      the output directories' name of the mobivision VDJ results.")
}

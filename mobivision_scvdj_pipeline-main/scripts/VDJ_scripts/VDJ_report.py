...
def vdj_report(input, configfile, vdjtype):
    ...
    header_info["订单编号"] = header_info.pop("Project_Num").split("-")[0]
    ...
    num = 0
	# mobivision
    if os.path.exists("%s/../../mobivision"  % (program_path)):
        num += 1
        os.makedirs(f"{outdir}/{num}.mobivision/")
        name = [str(i).split('/')[-2] for i in glob.glob("../../mobivision/*/outs")]
		
        # for j in name:
        #     os.makedirs("%s/%s.mobivision/%s" % (outdir, num, j))
        #     subprocess.call('ln -s %s/mobivision/%s/outs/* %s/%s.mobivision/%s ' % (program_path, j, outdir, num, j), shell=True)
        #     if os.path.exists("%s/%s.mobivision/%s/*.bam*" % (outdir, num, j)):
        #         subprocess.call('rm %s/%s.mobivision/%s/*.bam*' % (outdir, num, j), shell=True)
        #if len(name) >1:
        #    if os.path.exists("%s/mobivision/aggr"  % (program_path)):
        #        os.makedirs("%s/%s.mobivision/aggr" % (outdir, num))
        #        subprocess.call('ln -s %s/mobivision/aggr/*/outs/* %s/%s.mobivision/aggr ' % (program_path, outdir, num), shell=True)
        #        subprocess.call('rm  %s/%s.mobivision/aggr/aggregation.csv ' % ( outdir, num), shell=True)
        #    else:
        #        print("Can not find mobivision Aggr results!!!!!!!!!!!!!!!!")
        #        log_file.write("Can not find mobivision Aggr results!!!!!!!!!!!!!!!!" + "\n")
        #        sys.exit()
        subprocess.call('cp %s/../../mobivision/*%s.png %s/%s.mobivision/ ' % (program_path,vdjtype, outdir, num), shell=True)
        subprocess.call('cp -r %s/../../mobivision/*%s %s/%s.mobivision/ ' % (program_path,vdjtype, outdir, num), shell=True)
    else:
        print("Can not find mobivision results!!!!!!!!!!!!!!!!")
        log_file.write("Can not find mobivision results!!!!!!!!!!!!!!!!" + "\n")
  ...
  ### supplemental_material
	...
	# Define header info
    report = Report(f"MobiNova 单细胞 V(D)J 测序结题报告-{vdjtype}", title = f"MobiNova 单细胞 V(D)J 测序结题报告-{vdjtype}", header_info = dict(header_info), oe_welcome='''
####  感谢您选择欧易生物！
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
	...
	##########################项目摘要、快捷链接##########################	
    #link_refdiff = f"{num}.supplemental_material/MobiNova 单细胞 V(D)J 测序常见问题(FAQ).pdf"
    if sample_num == 1 :
    ...
    library_module_plot = library_module.add_plot('vdj_Experiment.png',caption ="MobiNova Genomics 单细胞 5' 端转录组及免疫组库测序实验技术流程",content="MobiNova Genomics 单细胞 5' 端转录组及免疫组库测序实验技术流程")
    ...
    QC_1_plots = sorted(list(glob.glob('1.mobivision/*.png')),reverse=False)

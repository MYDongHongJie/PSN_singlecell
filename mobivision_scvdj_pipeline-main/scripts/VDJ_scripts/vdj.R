    ...
    for (i in facetby){ 
        if (length(unique(seurat_ob@meta.data[,i]))>1) {
            ggtsne = DimPlot(object = seurat_ob, reduction = opt$reduct,pt.size = pointsize,group.by="topn_clonotype_id",split.by=i,ncol = 2) +
                     ...
                     ggplot2::theme( plot.title = ggplot2::element_text(hjust = 0.5)) +
                     ggplot2::scale_color_manual( values =  c(CustomCol2(1:(nlevel-1) ),"lightgrey"))
            ggplot2::ggsave(file.path(output_dir1,paste0("top_",topn,"_splitby_",i,"_clonotype_id",".pdf")), ggtsne, width=14)
            ggplot2::ggsave(file.path(output_dir1,paste0("top_",topn,"_splitby_",i,"_clonotype_id",".png")), ggtsne, width = 14 , dpi=1000)   
        }
    }
}

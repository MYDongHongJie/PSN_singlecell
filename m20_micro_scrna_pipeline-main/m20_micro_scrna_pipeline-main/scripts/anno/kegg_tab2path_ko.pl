...
########database file############
my $class7="/home/rna/RNA_works/Sge/denovo_RNAseq_v3/3.annotation/bin/bin/KEGGpathway_three_levels_v2_191022.xls";
my $Rscript="/data/software/Anaconda3/envs/mro-v3.4.3/bin/Rscript";
my $anno="/home/rna/RNA_works/Sge/denovo_RNAseq_v3/3.annotation/bin/bin/KO2definition_nonredundancy.txt";
...
p <- p + theme\(panel.background = element_rect\(fill=\"white\"\),panel.grid.minor = element_blank\(\),panel.grid.major = element_blank\(\),plot.background = element_rect\(fill=\"transparent\",colour=NA\) \)
...
sub USAGE {
	...
	Usage:
		...
		-mode        species mode : plant,animal,vert             must be given
		-h           Help document
USAGE
	print $usage;
	exit;

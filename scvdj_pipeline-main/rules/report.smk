...
#        celltypingdb = "/data/database/celltype_refdata/Ath_wangjiawei_celltype_genename.rds"
    ...
    if species.split("-")[0] in ["人"]:
        ...
        celltype_dict = {
            ...
            "HSC": "/gpfs/oe-database/celltype_refdata/report/GSE116530.rds",
...
# ref_species = ref_species_dict[config["report_params"]["celltypingdb"]] 
...
## MT list exist
MT_gene = False
if os.path.exists(f"{config['report']['scrna_Reference']}/MT_genelist.gmt") :
    with open(f"{config['report']['scrna_Reference']}/MT_genelist.gmt") as gmt:
        MT_gene = any([1  if line.find("percent.mito") == 0 else 0 for line in gmt])
## HB list exist
HB_gene = False
if os.path.exists(f"{config['report']['scrna_Reference']}/HB_genelist.gmt") :
    with open(f"{config['report']['scrna_Reference']}/HB_genelist.gmt") as gmt:
...
def all_input_report(wildcards):
    ...
    if config["report_params"]["module"]["celltyping"]:
        wanted_input.extend(
            expand([
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_celltyping_statistics.xls",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_celltyping_results.xls",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_simplified_celltyping_results.csv",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_celltyping_heatmap.png",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_celltyping_heatmap.pdf",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_celltyping_plot.png",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_main_celltyping_plot.pdf",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_top.main_celltyping_plot.png",
                "result/Reference_celltype/{ref_species}ref_{celltypingdb}_top.main_celltyping_plot.pdf"
                ],
    ...
    if config["report_params"]["module"]["report"]:
        ...
        wanted_input.extend([f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}/分析说明.html",
        "logs/upload_report_sor"])
        # wanted_input.extend(["result/rds/data_ob_v3.rds"])
    if config["report_params"]["module"]["report_upload"]:
        Project_Num = config['report']['Task_Num'].split("-")[0]
        wanted_input.extend([
            f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}.zip",
            f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}.zip.md5",
    ...
    if config["report_params"]["module"]["VDJ_report"]:
        for vdj_type in VDJ_Types:
            wanted_input.extend(
                expand([
                    f"logs/{vdj_type}_report.log",
                  ],vdj_type= vdj_type 
                )
            )
    if config["report_params"]["module"]["VDJ_scRNA_Conjoint"]:
        for vdj_type in VDJ_Types:
            wanted_input.extend(
                expand([
                    f"logs/{vdj_type}_vdj_scRNA_conjoint.log",
                    f"result/VDJ_aggr/{vdj_type}/vdj_scRNA_Conjoint_analysis/clonotype.metadata.xls"
                  ],vdj_type= vdj_type
                )
            )        
    return wanted_input
...
if config["report_params"]["module"]["VDJ_plot"] or config["report_params"]["module"]["VDJ_scRNA_Conjoint"]:
  include: "8.vdj.smk"
if config["report_params"]["module"]["report"] or config["report_params"]["module"]["report_upload"]:
  include: "9.report.smk"
if config["report_params"]["module"]["VDJ_report"] :
  include: "10.vdjreport.smk"

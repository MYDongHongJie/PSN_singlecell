...
rule report:
    ...
    output:
        html = f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}/分析说明.html"
    params:
        report = f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}/",
        config = "config/config.yaml",
    benchmark:
...
rule report_upload:
    ...
    """
    input:
        report = f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}/分析说明.html"
    output:
        protected(f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}.zip"),
        protected(f"result/report/{Project_Num}_Report_{time.strftime('%Y_%m_%d')}.zip.md5"),
        "result/report/report_upload.check"
    params:
        report_num = Project_Num,
        report = f"{Project_Num}_Report_{time.strftime('%Y_%m_%d')}"
    benchmark:

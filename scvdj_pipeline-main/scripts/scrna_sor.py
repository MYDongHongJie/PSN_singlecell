...
def project(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    desc.pop("Task_Num")
    desc.pop("scrna_Reference")
    desc.pop("vdj_Reference")
    desc.pop("Library")
    desc["Rawdata_Path"] = ','.join(map(str, yaml_file["cellranger_params"]["raw_data_obs_address"]))
    if yaml_file["cellranger_params"]["include_introns"]==True:
...
#            samples = pd.Series()
#            for addr in yaml_file["cellranger_params"]["raw_data_obs_address"]:
...
#                        samples[i.strip().replace("/","")] = i.strip().replace("/","")
    ...
    for (sample) in samples.index:
        ...
        QC=pd.read_csv(f"result/cellranger/{sample}/outs/per_sample_outs/{sample}/metrics_summary.csv", dtype=str)
        scrna_df = QC[QC['Library Type']=="Gene Expression"]
        scrna_df=scrna_df.iloc[:,4:].T
        scrna_df= scrna_df.reset_index(drop=True)
        scrna_df.columns = scrna_df.iloc[0,]
        scrna_df = scrna_df[1:]
        # rename and reorder 
        desc=pd.DataFrame(scrna_df.values.T, index= scrna_df.columns.str.replace(" ", "_"), columns=scrna_df.index).to_dict()
        desc =list(desc.values())[0]
        ...
        desc["Reads_Mapped_to_Transcriptome(%)"] = desc.pop("Confidently_mapped_to_transcriptome")
        desc["Q30_Bases_in_RNA_Read(%)"] = desc.pop("Q30_RNA_read")
        desc["Reads_Mapped_to_Genome(%)"] = desc.pop("Mapped_to_genome")
        desc["Fraction_Reads_in_Cells(%)"] = desc.pop("Confidently_mapped_reads_in_cells")
        desc["Clean_Base"] = str(format(int(desc["Number_of_reads"].replace(",",""))*300 / 1000000000, '.2f')) + "G"
        desc["Number_of_Reads"] = desc.pop("Number_of_reads")
        desc["Median_Genes_per_Cell"] = desc.pop("Median_genes_per_cell")
        desc["Estimated_Number_of_Cells"]= desc.pop("Cells")
        desc=OrderedDict(desc)
        desc.update({'QC__Type':"cellranger"})
        ...
        desc=dict(desc)
        #desc["Chemistry"] ="", desc["Reference_path"], desc["Include_introns"] = open_html(os.path.abspath(f"result/cellranger/{sample}/outs/web_summary.html"))
        desc["Chemistry"] =yaml_file["report"]["Library"]
        desc["Reference_path"]=yaml_file["report"]["scrna_Reference"]
        desc["Include_introns"] =yaml_file["cellranger_params"]["include_introns"]
        keys_to_delete =['Confidently_mapped_to_intergenic_regions', 'Median_UMI_counts_per_cell', 'Number_of_short_reads_skipped', 'Estimated_number_of_cells', 'Median_reads_per_cell', 'Confidently_mapped_to_genome', 'Q30_barcodes', 'Mean_reads_per_cell', 'Confidently_mapped_to_intronic_regions', 'Q30_UMI', 'Sequencing_saturation', 'Confidently_mapped_antisense', 'Total_genes_detected', 'Confidently_mapped_to_exonic_regions', 'Valid_barcodes', 'Number_of_reads_in_the_library']
        desc = {key: desc[key] for key in desc if key not in keys_to_delete}
        # report-related
        if os.path.exists("result/Count_QC/cell_statitics_before_after_QC.xls"):
...
def database(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    if not os.path.exists(logfile):
        referenceDB_file = "%s/config.yaml" %(yaml_file["report"]["scrna_Reference"])
...
def software(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    :return:
    '''
    if yaml_file["cellranger_params"]["module"]["cellranger"] == True:
    ...
    if yaml_file["report_params"]["module"]["report"] == True:
        ...
        software_version.pop('Cell Ranger')
        ...
        with open (i,'r') as f:
            for line in f :
                ...
                if line.startswith("rule "):
                    ll= re.split('\s+|:',line)
                    rule_name=ll[1]
                    if rule_name not in ["cellranger_config_csv_prep","cellranger_aggr_prep","cellranger_sample_stat",
                                         "cellranger_check","cellranger_report"]:
                        threads=yaml_file['cpu'][rule_name]

...
def scrna_report(program_path, configfile, rds):
	...
	if not os.path.exists("cellranger" ):
		...
		sample_name = [str(i).split('/')[-3] for i in glob("cellranger/*/outs/per_sample_outs/*/count/sample_filtered_feature_bc_matrix" )]
		file_number ={i: len(os.listdir("cellranger/"+i+"/outs/per_sample_outs/"+i+"/count/")) for i in sample_name }
		if any([v!= 6 for k,v in file_number.items()]):
			logging("[cellranger module] cellranger/ 目录下各样本文件数 :" +  str(file_number) + "；每个样本per_sample_outs文件夹下的子文件夹count应有6个文件。默认为外来数据不提供cellranger结果。Skipping... \n[cellranger module] × " ,log_file,WARNING)
			module_num -= 1
		else:
			if len(sample_name) != Sample_Num:
				...
				for j in sample_name:
					os.makedirs(f"{outdir}/{module_num}.CellRanger/{j}/outs" )
					subprocess.call(f'ln -s {program_path}/cellranger/{j}/outs/* {outdir}/{module_num}.CellRanger/{j}/ ', shell=True)
					subprocess.call(f'rm {outdir}/{module_num}.CellRanger/{j}/config.csv', shell=True)
					subprocess.call(f'rm {outdir}/{module_num}.CellRanger/{j}/multi/*/*.bam*', shell=True)
					subprocess.call(f'rm {outdir}/{module_num}.CellRanger/{j}/per_sample_outs/{j}/*/*.bam*', shell=True)
					# png
					check_exist(
...
####################################################################################################
	...
	header_info["订单编号"] = header_info["Task_Num"].split("-")[0]
...
# [^topGO]:Alexa A, Rahnenfuhrer J. topGO: enrichment analysis for gene ontology. R package version 2.8,2010.''')
	...
	project_module.add_section(name="项目目录结构",description=report_Directory)
	#########################################################################
	## ========================= 2.5 SECTION 5 附录 ========================= 
	#########################################################################
	...
	with open(f'{config["report"]["scrna_Reference"]}/config.yaml') as f:

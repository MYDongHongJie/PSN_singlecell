...
def cellranger_report(program_path, configfile):
    ...
    log_file = open(f"{program_path}/Cellranger_report_log.txt", "w")
    ####################################################################################################
    ...
    module_num = 0
    tcr = f"{program_path}/cellranger_sum/TCR_cellranger_stat.xls"
    bcr = f"{program_path}/cellranger_sum/BCR_cellranger_stat.xls"
    ## ========================= 1.1 cellranger =========================
    if not os.path.exists("cellranger" ):
        ...
        sample_name = [str(i).split('/')[-3] for i in glob("cellranger/*/outs/per_sample_outs/" )]
        file_number ={i: len(os.listdir("cellranger/"+i+"/outs/")) for i in sample_name }
        if any([v!= 4 for k,v in file_number.items()]):
            logging("[cellranger module] cellranger/ 目录下各样本文件数 :" +  str(file_number) + "；每个样本应有4个文件。默认为外来数据不提供cellranger结果。Skipping... \n[cellranger module] × " ,log_file,WARNING)
            module_num -= 1
        else:
            if len(sample_name) != Sample_Num:
                ...
                for j in sample_name:
                    ...
                    subprocess.call(f'ln -s {program_path}/cellranger/{j}/outs/* {outdir}/{module_num}.CellRanger/{j}/outs ', shell=True)
                    #if os.path.exists(f"{outdir}/{module_num}.CellRanger/{j}/outs/*/*/*.bam" ):
                    subprocess.call(f'rm {outdir}/{module_num}.CellRanger/{j}/outs/*/*/*.bam*', shell=True)
                    subprocess.call(f'rm {outdir}/{module_num}.CellRanger/{j}/outs/config.csv', shell=True)
                    # png
                    check_exist(
                    ...
                    if os.path.exists(tcr ) :
                        check_exist(
                        file=f"cellranger/{j}_TCR.png",
                        destination= f"{outdir}/{module_num}.CellRanger/" ,
                        log_file=log_file
                    )
                    if os.path.exists(bcr ) :
                        check_exist(
                        file=f"cellranger/{j}_BCR.png",
                        destination= f"{outdir}/{module_num}.CellRanger/" ,
                        log_file=log_file
                    )                   
                # # aggr
...
####################################################################################################
    ...
    report = Report('10x单细胞转录组及免疫组库项目Cellranger结题报告', title = '10x单细胞转录组及免疫组库项目质控结题报告', header_info = dict(header_info), oe_welcome='''
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
    ...
    ## ========================= 2.1 SECTION1 项目概况 ========================= 
    ############################################################################
    summ = pd.read_csv(f"{program_path}/cellranger_sum/scrna_cellranger_stat.xls",index_col=0, sep='\t', thousands=',')
    if summ.shape[1] > 1 :
        if not os.path.exists(tcr ) :
            summ_bcr = pd.read_csv(bcr,index_col=0, sep='\t')
            QC_description = f'''本次分析完成 {summ.shape[1]} 个样本的单细胞转录组及免疫组库测序，各样本 Cell Ranger 定量质控的单细胞转录组高质量细胞数分布在 {min(summ.loc["Cells",]).replace(',','')} ~ {max(summ.loc["Cells",]).replace(',','')} 个， 免疫组库 BCR 高质量细胞数分布在{min(summ_bcr.loc["Estimated number of cells",]).replace(',','')} ~ {max(summ_bcr.loc["Estimated number of cells",]).replace(',','')} 个。'''
        elif not os.path.exists(bcr ) :
            summ_tcr = pd.read_csv(tcr,index_col=0, sep='\t')
            QC_description = f'''本次分析完成 {summ.shape[1]} 个样本的单细胞转录组及免疫组库测序，各样本 Cell Ranger 定量质控的单细胞转录组高质量细胞数分布在 {min(summ.loc["Cells",]).replace(',','')} ~ {max(summ.loc["Cells",]).replace(',','')} 个， 免疫组库 TCR 高质量细胞数分布在{min(summ_tcr.loc["Estimated number of cells",]).replace(',','')} ~ {max(summ_tcr.loc["Estimated number of cells",]).replace(',','')} 个。'''
        else:
            summ_tcr = pd.read_csv(tcr,index_col=0, sep='\t')
            summ_bcr = pd.read_csv(bcr,index_col=0, sep='\t')
            QC_description = f'''本次分析完成 {summ.shape[1]} 个样本的单细胞转录组及免疫组库测序，各样本 Cell Ranger 定量质控的单细胞转录组高质量细胞数分布在 {min(summ.loc["Cells",]).replace(',','')} ~ {max(summ.loc["Cells",]).replace(',','')} 个， 免疫组库 TCR 高质量细胞数分布在{min(summ_tcr.loc["Estimated number of cells",]).replace(',','')} ~ {max(summ_tcr.loc["Estimated number of cells",]).replace(',','')} 个,免疫组库 BCR 高质量细胞数分布在{min(summ_bcr.loc["Estimated number of cells",]).replace(',','')} ~ {max(summ_bcr.loc["Estimated number of cells",]).replace(',','')} 个。'''
    else:
        if not os.path.exists(tcr ) :
            summ_bcr = pd.read_csv(bcr,index_col=0, sep='\t')
            QC_description = f'''本次分析完成 {summ.shape[1]} 个样本的单细胞转录组及免疫组库测序，各样本 Cell Ranger 定量质控的单细胞转录组高质量细胞数为 {min(summ.loc["Cells",]).replace(',','')} 个， 免疫组库 BCR 高质量细胞数为 {min(summ_bcr.loc["Estimated number of cells",]).replace(',','')} 个。'''
        elif not os.path.exists(bcr ) :
            summ_tcr = pd.read_csv(tcr,index_col=0, sep='\t')
            QC_description = f'''本次分析完成 {summ.shape[1]} 个样本的单细胞转录组及免疫组库测序，各样本 Cell Ranger 定量质控的单细胞转录组高质量细胞数为 {min(summ.loc["Cells",]).replace(',','')} 个， 免疫组库 TCR 高质量细胞数为{min(summ_tcr.loc["Estimated number of cells",]).replace(',','')} 个。'''
        else:
            summ_tcr = pd.read_csv(tcr,index_col=0, sep='\t')
            summ_bcr = pd.read_csv(bcr,index_col=0, sep='\t')
            QC_description = f'''本次分析完成 {summ.shape[1]} 个样本的单细胞转录组及免疫组库测序，各样本 Cell Ranger 定量质控的单细胞转录组高质量细胞数为 {min(summ.loc["Cells",]).replace(',','')} 个， 免疫组库 TCR 高质量细胞数为 {min(summ_tcr.loc["Estimated number of cells",]).replace(',','')} 个,免疫组库 BCR 高质量细胞数为 {min(summ_bcr.loc["Estimated number of cells",]).replace(',','')} 个。'''

    project_module = report.add_section('项目概况',description = QC_description)
    # register_info = oeweb_register(project_id=program_num, target_url='https://cloud.oebiotech.com/task/category/scrna', note='本报告包含项目基本分析内容，如需快速做细胞表达可视化等分析(数据默认保留半年)')
    # if register_info:
    ...
    ## 2.1
    ## ========================= 2.1.1 项目摘要、快捷链接  =========================  
    # project_module.add_section(name="项目摘要",description = QC_description)
    
    
    ## ========================= 2.1.2 项目快捷链接 =========================
    # link_faq = 'supplemental_material/单细胞转录组常见问题(FAQ).pdf'
    ...
    ###############################################################################
    ## ========================= 2.2.1 建库测序流程 =========================  
    library_module = report.add_section('实验技术流程_5转录组')
    ...
    ## ========================= 2.3 SECTION3 生信分析流程 ========================= 
    ###############################################################################
    analysis_module = report.add_section('Cell Ranger 分析流程')
    analysis_module.add_comment('Cell Ranger 分析流程图如下：')
    analysis_module.add_plot('Cellranger.png', caption = 'Cell Ranger 分析流程',content='Cell Ranger 分析流程图')
    ###############################################################################
    ## ========================= 2.4 SECTION4 项目分析结果 ========================= 
    ###############################################################################
    ...
    if len(list(glob("*.CellRanger"))) != 0:
        ...
        Cellranger = result_module.add_section('基因定量质控_cellranger_mutil')
        ...
        #Cellranger.add_table("../../cellranger_sum/metrics_summary_stat.csv", caption = '样本细胞质量统计结果')
        Cellranger.add_table('vdj_qc_stat.txt', caption = '样本细胞质量统计结果说明')
    ...
    # ## ========================= 2.5.5 AI修图使用说明 ========================= 
    # link_ai = 'supplemental_material/AI使用说明.html'
    ...
    software = software[software.Software.isin(['Cell Ranger'])]

...
option_list = list(
    make_option( c("--input", "-i"), type = "character", default = "result/cellranger",
                 help = "the input directory: cellranger result directory"),
...
if (all(file.exists(Sys.glob(file.path(vdj_path,assay_metadata$sampleid))) )){
  ...
  clonotypes_tbs = file.path(vdj_path,assay_metadata$sampleid,"outs","per_sample_outs",assay_metadata$sampleid,type_path,"clonotypes.csv")
  names(clonotypes_tbs) = paste0(assay_metadata$sampleid,"_",type)
  filtered_contig_tbs = file.path(vdj_path,assay_metadata$sampleid,"outs","per_sample_outs",assay_metadata$sampleid,type_path,"filtered_contig_annotations.csv")
  names(filtered_contig_tbs) = paste0(assay_metadata$sampleid,"_",type)
  vdj_gene_location = file.path(vdj_path,assay_metadata$sampleid,"outs","per_sample_outs",assay_metadata$sampleid,type_path,"airr_rearrangement.tsv")

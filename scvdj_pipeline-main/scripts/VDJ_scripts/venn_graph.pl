...
sub VennDiagram{
	...
	print R ".libPaths(\"/home/lipeng/R/x86_64-conda-linux-gnu-library/4.0\")\n";
#	print R ".libPaths(\"/home/lipeng/miniconda3/lib/R/library\")\n";
...
file.remove(dir(\".\", pattern=\"^VennDiagram.*log\$\"))\n";
	close R;
	system(" /home/lipeng/miniconda3/bin/Rscript $outdir/draw_venn.r && rm $outdir/draw_venn.r");
}
...
upset(data, nsets=datnum, nintersects=NA, number.angles=30, point.size=2, line.size=1, mainbar.y.label=\"Intersection clonetype number\", sets.x.label=\"Total clonetype number\", text.scale=c(1.3,1.3,1,1,1.3,1), mb.ratio=c(0.55, 0.45), order.by=\"freq\", show.numbers=\"yes\", sets.bar.color=rainbow(datnum))
dev.off()
...
sub Petals{
	...
	print SVGFILE $out;
	close SVGFILE;
	system("/home/lipeng/miniconda3/bin/cairosvg -b white $prefix\.svg -o $prefix\.png");
}

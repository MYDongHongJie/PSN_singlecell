...
def cellarnger_check(stat, html, Estimated_Number_of_Cells, Q30_Base_in_RNA_Read, Fraction_Reads_in_Cells, Median_Genes_per_Cell, Number_of_Reads, Reads_Mapped_Confidently_to_Transcriptome,Reads_Mapped_to_Genome, Project_Num, Species, Customer, Project_management, Laboratory, include_introns, remark, generate_QC_report, qc_report_folder):
    """
    ...
    Mapped_genome_min=int(Reads_Mapped_to_Genome[0].replace("%",""))
    Antisense_stand = 10
    ...
    for id in range(0,df.shape[0]):
        ...
        Mapped_genome_Sam = float( df.loc[sampleid]["Reads Mapped to Genome"].replace("%", ""))
        if int(Number_of_Reads.replace("M", "")) != 33:
            ...
            if ( Confi_Trome_Sam < Confi_Trome_min):
                ...
                warning_flag = True
             ### Reads_Mapped_to_Genome ###
            if ( Mapped_genome_Sam < Mapped_genome_min): 
                sample_error += "，Reads Mapped to Genome 低于 " + str(Mapped_genome_min) +  "%, 建议进行内部各环节排查。（请先做污染检测确认物种是否正确，若物种无误，附上污染检测结果再反馈邮件）"
                error_flag = True
    ...
    if generate_QC_report:
        if global_error_flag:
            header_flag="质控异常和项目报告"
            header = "【" + header_flag + "】"+ Project_Num + " " + Customer + "老师 " + "单细胞转录组项目质控反馈和项目报告"
        else:
            header_flag="质控反馈和项目报告"
            header = "【" + header_flag + "】"+ Project_Num + " " + Customer + "老师 " + "单细胞转录组项目质控反馈和项目报告"
    ...
    if generate_QC_report:
        QC_report_text = "\n该项目为只质控不分析项目，完整QC report已上传云报告。\n"
        info += QC_report_text
...
if __name__ == '__main__':
    ...
    Reads_Mapped_to_Genome = check_dict["Reads_Mapped_to_Genome"]
    ...
    generate_QC_report = config["email"]["generate_QC_report"]
    qc_report_folder = config["email"]["qc_report_folder"]
    ...
    (info,header)=cellarnger_check(stat, html, Estimated_Number_of_Cells, Q30_Base_in_RNA_Read, Fraction_Reads_in_Cells, Median_Genes_per_Cell, Number_of_Reads, Reads_Mapped_Confidently_to_Transcriptome,Reads_Mapped_to_Genome, Project_Num, Species, Customer, Project_management, Laboratory, include_introns, remark, generate_QC_report, qc_report_folder)

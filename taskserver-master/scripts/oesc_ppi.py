...
def main(inputfile, species, prefix, noft, outputdir):
    ...
    if results_data.shape[0] > 1:
        if "Regulation" in diffgene.columns:
            uptop_diff = diffgene[diffgene['Regulation'] == "Up"].sort_values(by="FoldChange", ascending=False)[0:noft]
            downtop_diff = diffgene[diffgene['Regulation'] == "Down"].sort_values(by="FoldChange", ascending=False)[
                           len(diffgene[diffgene['Regulation'] == "Down"]) - noft:len(
                               diffgene[diffgene['Regulation'] == "Down"])]
            top_diff = uptop_diff.append(downtop_diff)
            get_network_image(top_diff, species, prefix + ".string_protein-protein-interaction.svg")
            top_gene =  list(top_diff["gene"])
            top_mapping_result = mapping(top_gene, species, prefix +".mapping_results-top25.tsv")
            top_results_data = get_network(top_mapping_result, species, prefix +".string_protein-protein-interaction-top25.tsv")
        else:
        ...
        ###################################################################################################################
        # change networks color by Regulation information
        if "Regulation" in diffgene.columns:
            svg_files = prefix + ".string_protein-protein-interaction.svg"
            cmd = f"oesc_change_string_colors.py  -s {svg_files}"
            ...
            cmd = f"oesc_change_string_colors.py -s {svg_files} -c color_table.tsv "
            subprocess.run(cmd, shell=True, check=True)
            ...
            top_mapping_result.columns = ['gene_tihuan','num','ens','len','Mus','preferredName_A','info']
            top_results_data.columns = ["stringId_A","stringId_B","preferredName_A","preferredName_B","ncbiTaxonId","score","nscore","fscore","pscore","ascore","escore","dscore","tscore"]
            top_results_data.drop([0],inplace=True)
            a=top_results_data.merge(top_mapping_result,on=["preferredName_A"],how="left")
            top_mapping_result.columns = ['gene_tihuan','num','ens','len','Mus','preferredName_B','info']
            b=a.merge(top_mapping_result,on=["preferredName_B"],how="left")
            c=b[["stringId_A","stringId_B","gene_tihuan_x","gene_tihuan_y","ncbiTaxonId","score","nscore","fscore","pscore","ascore","escore","dscore","tscore"]]
            c.columns = ["stringId_A","stringId_B","preferredName_A","preferredName_B","ncbiTaxonId","score","nscore","fscore","pscore","ascore","escore","dscore","tscore"]
            c.to_csv(prefix + ".ppi_network.tsv",sep='\t',index=False)

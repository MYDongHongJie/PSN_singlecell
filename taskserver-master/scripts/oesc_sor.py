...
def QC(compact,yaml_file,user,pro_type,info_type,logdir):
    ...
    # # report - Count_QC
    # if os.path.exists("result/Count_QC/cell_count_before_after_QC.xls"):
        # Count_QC = pd.read_csv("result/Count_QC/cell_count_before_after_QC.xls", dtype=str,sep="\t",index_col=0)
    ...
    ## main
    QC=pd.read_csv(run_dir+'/metrics_summary_stat.csv', dtype=str)
    desc0=pd.DataFrame(QC.values.T, index= QC.columns.str.replace(" ", "_"), columns=QC.index).to_dict()
    for i in range(len(desc0)):
        ...
        desc=dict(desc)
        desc["Chemistry"], desc["Reference_path"], desc["Include_introns"] = open_html(run_dir+"/"+sample+"_summary.html")
        # report-related
        # if os.path.exists("result/Count_QC/cell_count_before_after_QC.xls"):

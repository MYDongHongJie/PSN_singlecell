...
# }
#=======================================================================================================================
statisc_summary<- function(starsolo, samplelist, filtered, outdir){
    # lapply(samplelist, function(sample) {
    ...
    summary <- lapply(1:length(samplelist),function(i) {
              sample<-samplelist[i]
              summary_file<-glue::glue("{starsolo}/{sample}/{sample}_Solo.out/GeneFull/Summary.csv")
              data <-read.table(summary_file,sep=',',header=F,row.names=1)
              colnames(data) <- c(sample)
              if(filtered=="re-filtered"){
                  counts_mtx <- Seurat::Read10X(glue::glue("{dirname(summary_file)}/filtered/"))
                  data_ob <- Seurat::CreateSeuratObject(counts = counts_mtx, assay = "RNA")
                  data["Estimated Number of Cells",] <-dim(data_ob@assays$RNA@data)[2]
                  data["UMIs in Cells",] <- sum(data_ob@meta.data$nCount_RNA)
                  data["Mean UMI per Cell",] <- round(mean(data_ob@meta.data$nCount_RNA))
                  data["Median UMI per Cell",] <-round(median(data_ob@meta.data$nCount_RNA))
                  data["Mean GeneFull per Cell",] <-round(mean(data_ob@meta.data$nFeature_RNA))
                  data["Median GeneFull per Cell",] <-round(median(data_ob@meta.data$nFeature_RNA))
                  data["Total GeneFull Detected",] <- dim(data_ob@assays$RNA@data)[1]
                  #data<-data %>% dplyr::filter(!row_number() %in% c(10,11, 12, 13))
              }
              data })  %>%
              bind_cols %>%
              t()%>%
    ...
    summary <- dplyr::select(summary,-c("Reads With Valid Barcodes","Reads Mapped to GeneFull: Unique+Multiple GeneFull","Q30 Bases in CB+UMI"))
    write_tsv(summary, file=paste0(outdir, "/", "STARsolo.statistic.tsv"))
}
#=======================================================================================================================
...
"-s", "--metadata", default="config/samples.csv", help="样本信息列表[default \"%(default)s\"]")
sub_email$add_argument(
"-i", "--starsolo", default="result/1.STARsolo/", help="starsolo输出目录，[default \"%(default)s\"]")
sub_email$add_argument(
"-f", "--filtered", default="filtered", help="starsolo结果是否重新进行过滤，filtered 或者re-filtered [default \"%(default)s\"]")
# sub_email$add_argument(
...
if ("summary" %in% args) {
    ...
    if (opt$sub_name == "summary") {
        ...
        statisc_summary(opt$starsolo, samplelist,opt$filtered,output_dir)
        #=======================================================================================================================

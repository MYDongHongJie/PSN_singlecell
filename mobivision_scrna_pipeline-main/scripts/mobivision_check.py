...
# Author Liuhongyan
# Modified luyao;gcy
# Date: 2020/1/24 13:31;2022/11/12
# Desc   : This script is used to get mobivision summary information 
check_yaml="""
...
parser.add_argument("-s", "--stat",help="all samples mobivision summary")
parser.add_argument("-m", "--html", help="all samples mobivision summary html, 用 , 分隔")
parser.add_argument("-p", "--png", help="all samples mobivision summary png, 用 , 分隔")
parser.add_argument("-d", "--csv",help="all samples mobivision summary csv, 用 , 分隔")
...
def mobivision_check(stat,mobivision_only,Estimated_Number_of_Cells,Q30_Base_in_RNA_Read,Fraction_Reads_in_Cells,Median_Genes_per_Cell,Number_of_Reads,Reads_Mapped_Confidently_to_Transcriptome,Project_Num,Species,Customer,Project_management,Laboratory,raw_data_obs_address,include_introns):
    """
    mobivision check result 
    
    """
    ### 各比较指标 ### 
    Med_Genes_Cell_stand = int(Median_Genes_per_Cell[1])
    Med_Genes_Cell_min = int(Median_Genes_per_Cell[0])
    Med_Genes_Cell_min_warning = int(Median_Genes_per_Cell[2])
    Est_Num_Cells_max_stand = int(Estimated_Number_of_Cells[1])
    Est_Num_Cells_min_stand = int(Estimated_Number_of_Cells[0])
    Est_Num_Cells_min_warning = int(Estimated_Number_of_Cells[2])
    ...
    forcecell_numcells_sampleid = []
    forcecell_medgenes_sampleid = []
    mobivision_only_header = ''
    info = ''
    all_sample_text = ''
    ...
    for sampleid in df.index: 
        ...
        Frac_Reads_Cells_Sam=float(df.loc[sampleid]["Fraction of Unique Reads in Cells"])
        Number_Reads_Sam=int(str(df.loc[sampleid]["Number of Raw Reads"]).replace(",",""))/1000000
        ...
        if int(Number_of_Reads.replace("M", "")) != 33:
            ...
            if ( Est_Num_Cells_Sam > Est_Num_Cells_max_stand ):
                ...
                error_flag = True
            ### Estimated Number of Cells 提示分析人员判断是否进行force cell  
            if ( Est_Num_Cells_Sam < Est_Num_Cells_min_warning ):
                forcecell_numcells_sampleid.append(sampleid) 
            ...
            if ( Med_Genes_Cell_Sam < Med_Genes_Cell_min ):
                ...
                warning_flag = True
            ### Median Genes per Cell 提示分析人员判断是否进行force cell     
            if ( Med_Genes_Cell_Sam < Med_Genes_Cell_min_warning ):
                forcecell_medgenes_sampleid.append(sampleid)  
               

            ### Fraction Reads in Cells ### 
            if ( Frac_Reads_Cells_Sam < Frac_Reads_Cells_min): 
                sample_error += "，Fraction of Unique Reads in Cells 低于 " + str(Frac_Reads_Cells_stand)+  "%"
                error_flag = True
            elif ( Frac_Reads_Cells_min <= Frac_Reads_Cells_Sam < Frac_Reads_Cells_stand ):
                sample_warning += "，Fraction of Unique Reads in Cells 稍低于 " + str(Frac_Reads_Cells_stand) +  "%"
                warning_flag = True
            ### Antisense_Sam
           # if (include_introns == "included" and Antisense_Sam > Antisense_stand ) :
    ...
    if mobivision_only:
        ...
        info = Project_management + '：\n    您好。\n    ' + Task_Num + " " + Customer + "老师 墨卓单细胞转录组项目 " + \
        ...
        all_samples = ", ".join(list(df.index))
        info = Project_management + '：\n    您好。\n    ' + Project_Num + " " + Customer + "老师 墨卓单细胞转录组项目 " + \
               str(len(df.index)) + " 个样本 (" +  all_samples + ") 质控, " + "物种：" + Species + "，质控结果如下：\n" + all_sample_text + "    详情见附件。\n"
               
    if forcecell_numcells_sampleid:
       forcecell_samples = "、 ".join(forcecell_numcells_sampleid)
       forcecell_error =   '\n    如下图所示：' + \
           forcecell_samples + " 样本" + "细胞数低于 " + str(Est_Num_Cells_min_warning) + ", 还请先判断是否有force cell调整空间,再反馈项目部! \n"
       info += forcecell_error
    elif forcecell_medgenes_sampleid:
       forcecell_samples = "、 ".join(forcecell_medgenes_sampleid)
       forcecell_error =   '\n    如下图所示：' + \
           forcecell_samples + " 样本" + "基因中位值低于 " + str(Med_Genes_Cell_min_warning) + ", 还请先判断是否有force cell调整空间,再反馈项目部! \n"
       info += forcecell_error
...
def send_email(emailto,header,context_html,png,csv,info):
    '''
        邮件发送
    '''
    ...
    for index,png_file in enumerate(png_list):
        with open(png_file, 'rb') as image:
            ...
            #image_info.add_header('Content-Id', '<image{}>'.format(index + 1))
    ...
    for id in range(len(array)):
        ...
        message.attach(att)
    csv_list = csv.split(",")
    for id in range(len(csv_list)):
        s_csv = csv_list[id]
        att_csv = MIMEText(open(s_csv, 'rb').read(), 'plain', 'utf-8')
        csv_name=os.path.abspath(s_csv).split("/")[-1]
        att_csv["Content-Type"] = 'application/octet-stream'
        att_csv["Content-Disposition"] = 'attachment; filename="' + csv_name + '"'
        message.attach(att_csv)
...
if __name__ == '__main__':
    ...
    raw_data_obs_address = config["mobivision_params"]["raw_data_obs_address"][0]
    mobivision_only = config["mobivision_params"]["module"]["mobivision_report"]
    include_introns =  config["mobivision_params"]["intron"]
    Number_of_Reads = config["mobivision_params"]["Number_of_Reads"]

    #if "mobivision" in step:
    (info,header)=mobivision_check(stat,mobivision_only,Estimated_Number_of_Cells,Q30_Base_in_RNA_Read,Fraction_Reads_in_Cells,Median_Genes_per_Cell,Number_of_Reads,Reads_Mapped_Confidently_to_Transcriptome,Project_Num,Species,Customer,Project_management,Laboratory,raw_data_obs_address,include_introns)

    send_email(emailto,header,html,png,csv,info)

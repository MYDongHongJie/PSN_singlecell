        ...
        if sample in os.listdir(f"{outdir}/"): 
            ...
            for i in {"1", "2"}:
                ...
                for fastq in fq_list:
                    src = fastq
                    dst = f'{outdir}/{sample}/{sample}_S{j}_L001_R{i}_001.fastq.gz'
                    fastq_show=fastq.replace(f"{outdir}/","")
                    des_show=f"{sample}/{sample}_S{j}_L001_R{i}_001.fastq.gz"
...
def run(configfile,outdir):
    ...
    addresses = config["mobivision_params"]["raw_data_obs_address"]
    samples_config = config["mobivision_params"]["samples"]
    ## get sample names
    if type(samples_config) == list:
        ...
        sys.exit("\033[31;1m"+"ERROR: mobivision_params:samples can only be string 'all' or a list of sample names."+"\033[0m")

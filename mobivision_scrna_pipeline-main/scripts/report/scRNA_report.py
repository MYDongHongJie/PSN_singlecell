...
def scrna_report(program_path, configfile, rds):
    ...
    if not os.path.exists("mobivision" ):
        ...
        sample_name = [str(i).split('/')[-2] for i in glob("mobivision/*/outs" )]
        #file_number ={i: len(os.listdir("mobivision/"+i+"/outs/")) for i in sample_name }
        #if any([v!= 9 and v !=11 for k,v in file_number.items()]):
        #	logging("[mobivision module] mobivision/ 目录下各样本文件数 :" +  str(file_number) + "；每个样本应有9或11个文件。默认为外来数据不提供mobivision结果。Skipping... \n[mobivision module] × " ,log_file,WARNING)
        #	module_num -= 1
        #else:
    ...
    if not os.path.exists("Diffexp" ):
        ...
        if not os.path.exists("Diffexp/ppi" ):
            ...
            check_file_num("Diffexp/ppi", f"{outdir}/{module_num}.ppi",ndiffexp*13+1,log_file)
    ...
    subprocess.call(f'cp -r {scriptdir}/墨卓欧易生物单细胞转录组报告树状图.pdf {outdir}/', shell=True)
...
####################################################################################################
    ...
    header_info = {key: config["report"][key] for key in config["report"] if
               key in ["Customer", "Species", "Sample_Num", "Library","Task_Num"]}
...
6. 本提示可以点击右上方的不再显示，则后续打开网页均不会显示本提示。
    ...
    if len(list(glob("*.MobiVision"))) != 0:
        ...
        MobiVision.add_table('scRNA_qc_stat.txt', caption = '样本细胞质量统计结果说明')
...
每个细胞中的平均基因数分布在 {format(min(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')} ~ {format(max(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')}''')
            ...
            report_summary_list.append(f'''
本次分析共完成 1 个样本单细胞转录组测序,
各样本定量质控的高质量细胞数分布在 {min(summ['Total_cells_beforeQC'])} 个，
经剔除双细胞、多细胞和凋亡细胞等质控后，最终获得的细胞数目分布在 {min(summ['Total_cells_after'+lastQCstep]) } 个，
每个细胞中的平均 UMI 数分布在 {format(min(summ['mean_nCount_RNA_after'+lastQCstep].astype(float)),'.0f')} ，
每个细胞中的平均基因数分布在 {format(min(summ['mean_nFeature_RNA_after'+lastQCstep].astype(float)),'.0f')} ''')
        ...
        try:
            ...
            QC_description = f'''在 MobiVision 初步质控的基础上进一步对实验数据进行质控，剔除多细胞、双细胞或者未结合上细胞的数据，进行下游分析。
...
\n质控前后每个细胞的基因数量（nGene）、UMI数量（nUMI）、单位UMI内基因数目的比例（log10GenesPerUMI）{mito_show}{HB_show}的小提琴图展示如下：
        '''
            ...
            QC_description = f'''在 MobiVision 初步质控的基础上进一步对实验数据进行质控，剔除多细胞、双细胞或者未结合上细胞的数据。{roboust_linear_model_description}
...
录本数目异常过多的细胞很可能是由于对应的油包水微滴中包含多个细胞，需要通过设置合理的阈值将其过滤''')
    ...
    if len(list(glob("*.Diffexp"))) != 0:
        ...
        Diff_module.add_comment("基于以上差异结果，进一步根据差异倍数（FoldChange）及显著性检验（pval）筛选显著的差异表达基因，结果见:")
        Diff_module.add_comment("[{link}]({link})",link=link)
        # if FC12 == True:
        ...
        Diff_module.add_table(f'{Diffexp_folder[0]}/diffexp_results_stat.xls', 
            caption = '差异表达基因统计表')  
        #	headerdict={'Case':'实验组名','Control':'对照组名','Up_diff':'显著上调差异基因数量','Down_diff':'显著下调差异基因数量','Total_diff(pval<0.05&FoldChange>1.5)':'显著性差异基因总数量'})
        ...
        diff_gene_GO.add_table(f'{enrichment_folder[0]}/GO_enrichment/{diff_group[0]}/enrichment-go-*-Total.xls',
            caption='富集分析结果表格',
            show_rows=10)
            #headerdict={'id':'条目在Gene Ontology的登录号','term':'该条目的描述','category':'GO分类','ListHits':'该GO条目中差异基因数','ListTotal':'注释到GO的总差异基因数','PopHits':'注释到该条目中的所有基因数目','PopTotal':'注释到GO的总基因数','pval':'富集显著性p值','pval':'校正后的p值','Enrichment_score':'富集打分','Gene':'属于该条目的差异gene'})
...
#				description='图片说明：横轴是注释到各 Level2 通路的上调（下调）差异表达基因和所有注释到 KEGG 通路的上调（下调）差异表达基因总数的比值（%），纵轴表示 Level2 Pathway 的名称，柱子右边数字代表注释到该 Level2 Pathway 的上调（下调）差异表达基因数量。')
        ...
        if len(ppi_folder)>0: 
            ...
            ppi_module.add_table(ppi_table,caption='差异基因互作关系表',show_rows=10)
...
#				headerdict={'stringId_A':'基因A的stringId','stringId_B':'基因B的stringId','preferredName_A':'A基因名','preferredName_B':'B基因名','ncbiTaxonId':'NCBI的物种分类标识符','score':'结合不同渠道证据的概率矫正后的综合得分','nscore':'邻近得分（从基因间核苷酸计数计算）','fscore':'融合得分（来自其他物种的融合蛋白质）','pscore':'系统谱系的共存得分（源自相似的基因缺失/存在模式）','ascore':'共表达得分（源自由DNA阵列和类似技术测量相似的mRNA表达模式）','escore':'实验得分（来自实验数据，如亲和色谱）','dscore':'数据库得分（派生自各种数据库的精选数据）','tscore':'文本挖掘得分（源自摘要中同时存在的基因/蛋白质名称）'})
    ...
    link_Directory = '墨卓欧易生物单细胞转录组报告树状图.pdf'
    report_hyperlink_list.append(f'''[单细胞转录组常见问题（FAQ）]({link_faq})\n\n[报告解读视频]({link_video})\n''' )
    report_hyperlink = ''.join(report_hyperlink_list)
    report_Directory_list.append(f'''[墨卓欧易生物单细胞转录组报告树状图]({link_Directory})\n''' )
    ...
    ## ========================= 2.5.3 生信分析方法说明  ========================= 
    link3 = '10.supplemental_material/墨卓欧易生物单细胞转录组基础分析方法_中文.pdf'
    link4 = '10.supplemental_material/墨卓欧易生物单细胞转录组基础分析方法_英文.pdf'
    ...
    appendix_module.add_table('scRNA_software.txt', caption = '使用软件及版本')
    #########################################################################
    ## ========================= 2.6 SECTION 6 申明 =========================
    #########################################################################

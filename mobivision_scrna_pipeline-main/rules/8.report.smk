...
rule report:
    ...
    output:
        html = f"result/report/{config['report']['Task_Num'].split('-')[0]}_Report_{time.strftime('%Y_%m_%d')}/分析说明.html"
    params:
        report = f"result/report/{config['report']['Task_Num'].split('-')[0]}_Report_{time.strftime('%Y_%m_%d')}/",
        config = "config/config.yaml",
    benchmark:
...
rule report_upload:
    ...
    """
    input:
        report = f"result/report/{config['report']['Task_Num'].split('-')[0]}_Report_{time.strftime('%Y_%m_%d')}/分析说明.html"
    output:
        protected(f"result/report/{config['report']['Task_Num'].split('-')[0]}_Report_{time.strftime('%Y_%m_%d')}.zip"),
        protected(f"result/report/{config['report']['Task_Num'].split('-')[0]}_Report_{time.strftime('%Y_%m_%d')}.zip.md5"),
        "result/report/report_upload.check"
    params:
        report_num = config['report']['Task_Num'].split('-')[0],
        report = f"{config['report']['Task_Num'].split('-')[0]}_Report_{time.strftime('%Y_%m_%d')}"
    benchmark:

  ...
  delete table_obj["pdf"];
    ...
    $("[replace]").each(function(){
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    $(this).after( $(this).prop("outerHTML").replace(re, arr[i]).replace(/replace=\"\w+\"/,''));
                }
                $(this).remove();
    })
    ...
    $('.nav-pills-top a').click(function () {
		var parent = $(this).parent();
		if (parent.hasClass('nav-pills-one')) {
			$('.nav-pills-one > a').removeClass('active-color');
			$(this).addClass('active-color');
		} else if (parent.hasClass('nav-pills-two')) {
			$('.nav-pills-two > a').removeClass('active-color');
			$(this).addClass('active-color');
		} else if (parent.hasClass('nav-pills-three')) {
			$('.nav-pills-three > a').removeClass('active-color');
			$(this).addClass('active-color');
		} else if (parent.hasClass('nav-pills-four')) {
			$('.nav-pills-four > a').removeClass('active-color');
			$(this).addClass('active-color');
		}
	})
});
